...
# Required env: GH_TOKEN, PR_NUMBER, GH_REPO, PLOW_COMMAND
# Optional env: CHANGED_SINCE, INPUT_ROOT, PLOW_ANALYSIS_ARGS_FILE,
#   PLOW_ARTIFACTS_DIR
...
render_with_plow() {
  ...
  PLOW_COMMENT_ID="${PLOW_COMMENT_ID:-plow-results}" plow "${args[@]}" > "$output" 2> "$(artifact_path plow-comment-stderr.log)" || true
...
PR_COMMENT_FILE=$(artifact_path plow-pr-comment.md)
if render_with_plow pr-comment-github "$PR_COMMENT_FILE"; then
  COMMENT_BODY=$(cat "$PR_COMMENT_FILE")
  export MARKER="<!-- plow-id: ${PLOW_COMMENT_ID:-plow-results} -->"
  ...
  if gh_api_retry \
    ...
    echo "::warning::plow: failed to look up existing PR summary comment; posting a new one (may duplicate). stderr: ${_STDERR_HEAD} Re-run the job to retry. If persistent, check 'gh auth status' and repo permissions." >&2
