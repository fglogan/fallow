...
pub fn render_pr_comment(input: &PrCommentRenderInput<'_>) -> String {
    let marker = format!("<!-- plow-id: {} -->", input.marker_id);
...
mod tests {
    ...
    fn category_for_rule(rule_id: &str) -> &'static str {
        match rule_id {
            "plow/code-duplication" => "Duplication",
            "plow/high-complexity" => "Health",
            "plow/unused-dependency" => "Dependencies",
            _ => "Dead code",
        }
    }
    ...
    fn renders_default_empty_comment() {
        let body = render_pr_comment(&PrCommentRenderInput {
            ...
            issues: &[],
            marker_id: "plow-results".to_owned(),
            max_comments: 50,
