...
/// Discriminator inputs that PRODUCED a finding's verdict (plow-rs/plow-cloud#321),
...
/// F4: these make the EXISTING Plow-owned discriminators legible; they are not
...
pub struct RuntimeCoverageReport {
    ...
    /// Whether an autonomous agent may act on this report (plow-rs/plow-cloud#316,
    ...
    /// Provenance an agent reads to self-attenuate confidence (plow-rs/plow-cloud#319,
    /// mirrors the cloud runtime-context `provenance`). F4: context only.
    pub provenance: RuntimeCoverageProvenance,
}

/// Provenance of a runtime-coverage report (plow-rs/plow-cloud#319), mirroring
