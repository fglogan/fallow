...
impl From<plow_core::changed_files::ChangedFilesError> for ChangedFilesError {
    fn from(error: plow_core::changed_files::ChangedFilesError) -> Self {
        match error {
            plow_core::changed_files::ChangedFilesError::InvalidRef(err) => Self::InvalidRef(err),
            plow_core::changed_files::ChangedFilesError::GitMissing(err) => Self::GitMissing(err),
            plow_core::changed_files::ChangedFilesError::NotARepository => Self::NotARepository,
            plow_core::changed_files::ChangedFilesError::GitFailed(stderr) => {
...
pub fn resolve_git_common_dir(cwd: &Path) -> Result<PathBuf, ChangedFilesError> {
    plow_core::changed_files::resolve_git_common_dir(cwd).map_err(ChangedFilesError::from)
}
...
pub fn try_get_changed_files(
    ...
    plow_core::changed_files::try_get_changed_files(root, git_ref).map_err(ChangedFilesError::from)
}
...
pub fn try_get_changed_files_with_toplevel(
    ...
    plow_core::changed_files::try_get_changed_files_with_toplevel(cwd, toplevel, git_ref)
...
pub fn try_get_changed_diff(root: &Path, git_ref: &str) -> Result<String, ChangedFilesError> {
    plow_core::changed_files::try_get_changed_diff(root, git_ref).map_err(ChangedFilesError::from)
}
...
pub fn get_changed_files(root: &Path, git_ref: &str) -> Option<FxHashSet<PathBuf>> {
    plow_core::changed_files::get_changed_files(root, git_ref)
}
...
pub fn filter_duplication_by_changed_files(
    ...
    plow_core::changed_files::filter_duplication_by_changed_files(report, changed_files, root);
}
...
mod tests {
    ...
    fn changed_files_error_converts_from_core_without_leaking_type() {
        let error = plow_core::changed_files::ChangedFilesError::GitFailed(
