...
/// state and should not leak into plow-owned git subprocesses.
pub const AMBIENT_GIT_ENV_VARS: &[&str] = plow_core::git_env::AMBIENT_GIT_ENV_VARS;
...
pub fn clear_ambient_git_env(cmd: &mut Command) -> &mut Command {
    plow_core::git_env::clear_ambient_git_env(cmd)
}
