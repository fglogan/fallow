...
use plow_types::results::{SecurityFinding, SecuritySeverity};
...
pub fn derive_security_severity(finding: &SecurityFinding) -> SecuritySeverity {
    plow_core::analyze::derive_security_severity(finding)
}
...
pub fn security_catalogue_title(kind: &str) -> Option<&'static str> {
    plow_core::analyze::security_catalogue_title(kind)
}
