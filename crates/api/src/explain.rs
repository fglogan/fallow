...
pub const CHECK_RULES: &[RuleDef] = &[
    ...
    RuleDef {
        id: "plow/missing-suppression-reason",
        category: "Suppressions",
        name: "Missing Suppression Reason",
        short: "Suppression comment omits a required reason",
        full: "A plow-ignore-next-line or plow-ignore-file suppression omits the explanatory reason required by the requireSuppressionReason rule. Add a short reason after the suppression token, or remove the suppression if the issue is no longer intentional.",
        docs_path: "explanations/dead-code#stale-suppressions",
    },
    RuleDef {
    ...
    RuleDef {
        id: "plow/unused-component-input",
        category: "Dead code",
    ...
    RuleDef {
        id: "plow/unused-component-output",
        category: "Dead code",
...
fn dead_code_alias_id(normalized: &str) -> Option<&'static str> {
    match normalized {
        ...
        "unused-component-inputs" | "unused-component-input" => Some("plow/unused-component-input"),
        "unused-component-outputs" | "unused-component-output" => {
            Some("plow/unused-component-output")
        }
        "unused-svelte-events" | "unused-svelte-event" => Some("plow/unused-svelte-event"),
        "unused-server-actions" | "unused-server-action" => Some("plow/unused-server-action"),
        "unused-load-data-keys" | "unused-load-data-key" => Some("plow/unused-load-data-key"),
        "prop-drilling" => Some("plow/prop-drilling"),
        "thin-wrapper" | "thin-wrappers" => Some("plow/thin-wrapper"),
        "duplicate-prop-shape" | "duplicate-prop-shapes" => Some("plow/duplicate-prop-shape"),
        ...
        "missing-suppression-reason" | "missing-suppression-reasons" => {
            Some("plow/missing-suppression-reason")
        }
        _ => None,
    }
}
...
fn member_import_rule_guide(id: &str) -> Option<RuleGuide> {
    Some(match id {
        ...
        "plow/unused-component-prop" => RuleGuide {
            ...
            how_to_fix: "Remove the unused prop, or reference it in the component (Vue: the script / template; React: the function body or JSX). If the prop is part of a deliberately-stable public component API, suppress the line with // plow-ignore-next-line unused-component-prop.",
        },
        ...
        "plow/unused-component-input" => RuleGuide {
            ...
            how_to_fix: "Remove the unused input, or read it in the template or class body. If the input is part of a deliberately-stable public component API, suppress the line with // plow-ignore-next-line unused-component-input.",
        },
        "plow/unused-component-output" => RuleGuide {
            ...
            how_to_fix: "Remove the unused output, or emit it from the class. If the output is part of a deliberately-stable public component API, suppress the line with // plow-ignore-next-line unused-component-output.",
        },
        "plow/unused-svelte-event" => RuleGuide {
            ...
            how_to_fix: "Remove the dispatched event, or listen for it on the component (<Child on:dead={...}> or forward it via <Child on:dead>). If the event is dispatched reflectively (a dynamic name) or is part of a deliberately-stable public component API, suppress the line with // plow-ignore-next-line unused-svelte-event.",
        },
        ...
        "plow/unused-load-data-key" => RuleGuide {
            ...
            how_to_fix: "Delete the unused key from the load() return (and skip its fetch), or wire a consumer (read data.<key> in +page.svelte, or page.data.<key> in a shared component). If the load fetch has a side effect you must keep, suppress the line with // plow-ignore-next-line unused-load-data-key.",
        },
        "plow/prop-drilling" => RuleGuide {
...
fn architecture_rule_guide(id: &str) -> Option<RuleGuide> {
    Some(match id {
        ...
        "plow/policy-violation" => RuleGuide {
            ...
            how_to_fix: "Replace the banned call, import, or effectful usage with the alternative named in the rule's message. To waive one rule, use `// plow-ignore-next-line policy-violation:<pack>/<rule-id>` or the file-level form. Use bare `policy-violation` only when you intend to suppress every rule-pack finding at that scope.",
        },
        ...
        "plow/missing-suppression-reason" => RuleGuide {
            example: "// plow-ignore-next-line unused-export appears without the required explanatory reason.",
...
fn catalog_rule_guide(id: &str) -> Option<RuleGuide> {
    Some(match id {
        ...
        "plow/unresolved-catalog-reference" => RuleGuide {
            ...
            how_to_fix: "If `available_in_catalogs` is non-empty, change the reference to one of those catalogs (e.g. `catalog:react18`). Otherwise add the package to the named catalog in the catalog source, or remove the catalog reference and pin a hardcoded version. For staged migrations where the catalog edit lands separately, add the (package, catalog, consumer) triple to `ignoreCatalogReferences` in your plow config.",
        },
...
pub fn serialize_explain_programmatic_json(
    issue_type: &str,
    mode: plow_output::RootEnvelopeMode,
    analysis_run_id: Option<&str>,
) -> Result<serde_json::Value, crate::ProgrammaticError> {
    let output = explain_issue_type(issue_type)?;
    plow_output::serialize_explain_json_output(output, mode, analysis_run_id).map_err(|error| {
...
#[must_use]
pub fn security_meta() -> plow_types::envelope::Meta {
    plow_output::security_meta(
        SECURITY_RULES
            .iter()
            .map(|rule| plow_output::SecurityRuleMeta {
                id: rule.id,
                name: rule.name,
...
mod tests {
    ...
    fn meta_value(meta: plow_types::envelope::Meta) -> Value {
        serde_json::to_value(meta).expect("metadata should serialize")
    }

    fn check_meta() -> Value {
        meta_value(plow_output::check_meta())
    }

    fn health_meta() -> Value {
        meta_value(plow_output::health_meta())
    }

    fn dupes_meta() -> Value {
        meta_value(plow_output::dupes_meta())
    }
    ...
    fn result_sarif_rule_ids_have_explain_metadata() {
        for contract in plow_output::issue_output_contracts() {
    ...
    fn health_and_dupes_meta_share_actions_field_definitions() {
        for meta in [health_meta(), dupes_meta()] {
            ...
            assert_eq!(
                ...
                plow_output::ACTIONS_FIELD_DEFINITION,
            );
            assert_eq!(
                defs["actions[].auto_fixable"].as_str().unwrap(),
                plow_output::ACTIONS_AUTO_FIXABLE_FIELD_DEFINITION,
            );
        }
    }
    ...
    fn coverage_setup_meta_has_docs_fields_enums_and_warnings() {
        ...
        assert_eq!(meta["docs_url"], plow_output::COVERAGE_SETUP_DOCS);
        assert!(
    ...
    fn coverage_analyze_meta_documents_data_source_and_action_vocabulary() {
        ...
        assert_eq!(meta["docs_url"], plow_output::COVERAGE_ANALYZE_DOCS);
    ...
    fn check_docs_url_valid() {
        assert!(plow_output::CHECK_DOCS.starts_with("https://"));
        assert!(plow_output::CHECK_DOCS.contains("dead-code"));
    }

    #[test]
    fn health_docs_url_valid() {
        assert!(plow_output::HEALTH_DOCS.starts_with("https://"));
        assert!(plow_output::HEALTH_DOCS.contains("health"));
    }

    #[test]
    fn dupes_docs_url_valid() {
        assert!(plow_output::DUPES_DOCS.starts_with("https://"));
        assert!(plow_output::DUPES_DOCS.contains("dupes"));
    }

    #[test]
    fn check_meta_docs_url_matches_constant() {
        let meta = check_meta();
        assert_eq!(meta["docs"].as_str().unwrap(), plow_output::CHECK_DOCS);
    }

    #[test]
    fn health_meta_docs_url_matches_constant() {
        let meta = health_meta();
        assert_eq!(meta["docs"].as_str().unwrap(), plow_output::HEALTH_DOCS);
    }

    #[test]
    fn dupes_meta_docs_url_matches_constant() {
        let meta = dupes_meta();
        assert_eq!(meta["docs"].as_str().unwrap(), plow_output::DUPES_DOCS);
    }
