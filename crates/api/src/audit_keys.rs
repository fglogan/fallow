...
fn dependency_location_key(location: &plow_types::results::DependencyLocation) -> &'static str {
    match location {
        plow_types::results::DependencyLocation::Dependencies => "unused-dependency",
        plow_types::results::DependencyLocation::DevDependencies => "unused-dev-dependency",
        plow_types::results::DependencyLocation::OptionalDependencies => {
            "unused-optional-dependency"
        }
    }
}

fn unused_dependency_key(item: &plow_types::results::UnusedDependency, root: &Path) -> String {
    format!(
        "{}:{}:{}",
...
fn invalid_client_export_key(
    item: &plow_types::results::InvalidClientExport,
    root: &Path,
) -> String {
    format!(
...
fn mixed_client_server_barrel_key(
    item: &plow_types::results::MixedClientServerBarrel,
    root: &Path,
) -> String {
    format!(
...
fn misplaced_directive_key(item: &plow_types::results::MisplacedDirective, root: &Path) -> String {
    format!(
        "misplaced-directive:{}:{}:{}",
        relative_key_path(&item.path, root),
        item.line,
...
fn unprovided_inject_key(item: &plow_types::results::UnprovidedInject, root: &Path) -> String {
    format!(
        "unprovided-inject:{}:{}",
        relative_key_path(&item.path, root),
        item.key_name
...
fn unrendered_component_key(
    item: &plow_types::results::UnrenderedComponent,
    root: &Path,
) -> String {
    format!(
...
fn unused_component_prop_key(
    item: &plow_types::results::UnusedComponentProp,
    root: &Path,
) -> String {
    format!(
...
fn unused_component_emit_key(
    item: &plow_types::results::UnusedComponentEmit,
    root: &Path,
) -> String {
    format!(
...
fn unused_component_input_key(
    item: &plow_types::results::UnusedComponentInput,
    root: &Path,
) -> String {
    format!(
...
fn unused_component_output_key(
    item: &plow_types::results::UnusedComponentOutput,
    root: &Path,
) -> String {
    format!(
...
fn unused_svelte_event_key(item: &plow_types::results::UnusedSvelteEvent, root: &Path) -> String {
    format!(
        "unused-svelte-event:{}:{}",
        relative_key_path(&item.path, root),
        item.event_name
...
fn unused_server_action_key(item: &plow_types::results::UnusedServerAction, root: &Path) -> String {
    format!(
...
fn unused_load_data_key_key(item: &plow_types::results::UnusedLoadDataKey, root: &Path) -> String {
    format!(
...
fn route_collision_key(item: &plow_types::results::RouteCollision, root: &Path) -> String {
    format!(
        "route-collision:{}:{}",
        relative_key_path(&item.path, root),
        item.url
...
fn dynamic_segment_name_conflict_key(
    item: &plow_types::results::DynamicSegmentNameConflict,
    root: &Path,
) -> String {
    format!(
...
fn unlisted_dependency_key(item: &plow_types::results::UnlistedDependency, root: &Path) -> String {
    let mut sites = item
        .imported_from
        .iter()
        .map(|site| {
...
fn unused_member_key(
    rule_id: &str,
    item: &plow_types::results::UnusedMember,
    root: &Path,
) -> String {
    format!(
        "{}:{}:{}:{}",
...
fn unused_catalog_entry_key(item: &plow_types::results::UnusedCatalogEntry, root: &Path) -> String {
    format!(
...
fn empty_catalog_group_key(item: &plow_types::results::EmptyCatalogGroup, root: &Path) -> String {
    format!(
        "empty-catalog-group:{}:{}:{}",
        relative_key_path(&item.path, root),
        item.line,
...
fn duplicate_export_key(
    item: &plow_types::output_dead_code::DuplicateExportFinding,
    root: &Path,
) -> String {
...
fn circular_dependency_key(
    item: &plow_types::output_dead_code::CircularDependencyFinding,
    root: &Path,
) -> String {
...
fn re_export_cycle_key(
    item: &plow_types::output_dead_code::ReExportCycleFinding,
    root: &Path,
) -> String {
...
fn policy_violation_key(
    item: &plow_types::output_dead_code::PolicyViolationFinding,
    root: &Path,
) -> String {
    format!(
        "policy-violation:{}:{}/{}:{}",
...
fn stale_suppression_key(item: &plow_types::results::StaleSuppression, root: &Path) -> String {
...
///
/// Sibling exhaustive sites: `plow_engine::filter_results_by_changed_files`,
...
struct DependencyFindingSlices<'a> {
    unused_dependencies: &'a [plow_types::output_dead_code::UnusedDependencyFinding],
    unused_dev_dependencies: &'a [plow_types::output_dead_code::UnusedDevDependencyFinding],
    unused_optional_dependencies:
        &'a [plow_types::output_dead_code::UnusedOptionalDependencyFinding],
    unlisted_dependencies: &'a [plow_types::output_dead_code::UnlistedDependencyFinding],
    type_only_dependencies: &'a [plow_types::output_dead_code::TypeOnlyDependencyFinding],
    test_only_dependencies: &'a [plow_types::output_dead_code::TestOnlyDependencyFinding],
}
...
struct FrameworkFindingSlices<'a> {
    unprovided_injects: &'a [plow_types::output_dead_code::UnprovidedInjectFinding],
    unrendered_components: &'a [plow_types::output_dead_code::UnrenderedComponentFinding],
    unused_server_actions: &'a [plow_types::output_dead_code::UnusedServerActionFinding],
    unused_load_data_keys: &'a [plow_types::output_dead_code::UnusedLoadDataKeyFinding],
    route_collisions: &'a [plow_types::output_dead_code::RouteCollisionFinding],
    dynamic_segment_name_conflicts:
        &'a [plow_types::output_dead_code::DynamicSegmentNameConflictFinding],
}
...
/// backwards-compatible aliases from `plow schema` `ts_alias` rows.
...
impl DeadCodeKeyCollector<'_> {
    ...
    fn add_all_findings(&mut self, results: &plow_types::results::AnalysisResults) {
        let plow_types::results::AnalysisResults {
            unused_files,
            unused_exports,
            unused_types,
            private_type_leaks,
...
impl<'a> DeadCodeKeyCollector<'a> {
    ...
    fn add_core_findings(
        ...
        unused_exports: &[plow_types::output_dead_code::UnusedExportFinding],
        unused_types: &[plow_types::output_dead_code::UnusedTypeFinding],
        private_type_leaks: &[plow_types::output_dead_code::PrivateTypeLeakFinding],
    ) {
    ...
    fn add_client_directive_findings(
        &mut self,
        invalid_client_exports: &[plow_types::output_dead_code::InvalidClientExportFinding],
        mixed_client_server_barrels: &[plow_types::output_dead_code::MixedClientServerBarrelFinding],
        misplaced_directives: &[plow_types::output_dead_code::MisplacedDirectiveFinding],
    ) {
    ...
    fn add_dependency_override_findings(
        &mut self,
        unused_dependency_overrides: &[plow_types::output_dead_code::UnusedDependencyOverrideFinding],
        misconfigured_dependency_overrides: &[plow_types::output_dead_code::MisconfiguredDependencyOverrideFinding],
    ) {
    ...
    fn add_member_findings(
        &mut self,
        unused_enum_members: &[plow_types::output_dead_code::UnusedEnumMemberFinding],
        unused_class_members: &[plow_types::output_dead_code::UnusedClassMemberFinding],
        unused_store_members: &[plow_types::output_dead_code::UnusedStoreMemberFinding],
    ) {
    ...
    fn add_component_contract_findings(
        &mut self,
        unused_component_props: &[plow_types::output_dead_code::UnusedComponentPropFinding],
        unused_component_emits: &[plow_types::output_dead_code::UnusedComponentEmitFinding],
        unused_component_inputs: &[plow_types::output_dead_code::UnusedComponentInputFinding],
        unused_component_outputs: &[plow_types::output_dead_code::UnusedComponentOutputFinding],
        unused_svelte_events: &[plow_types::output_dead_code::UnusedSvelteEventFinding],
    ) {
    ...
    fn add_graph_findings(
        &mut self,
        unresolved_imports: &[plow_types::output_dead_code::UnresolvedImportFinding],
        duplicate_exports: &[plow_types::output_dead_code::DuplicateExportFinding],
        circular_dependencies: &[plow_types::output_dead_code::CircularDependencyFinding],
        re_export_cycles: &[plow_types::output_dead_code::ReExportCycleFinding],
    ) {
    ...
    fn add_boundary_findings(
        &mut self,
        boundary_violations: &[plow_types::output_dead_code::BoundaryViolationFinding],
        boundary_coverage_violations: &[plow_types::output_dead_code::BoundaryCoverageViolationFinding],
        boundary_call_violations: &[plow_types::output_dead_code::BoundaryCallViolationFinding],
        policy_violations: &[plow_types::output_dead_code::PolicyViolationFinding],
        stale_suppressions: &[plow_types::results::StaleSuppression],
    ) {
    ...
    fn add_catalog_findings(
        &mut self,
        unresolved_catalog_references: &[plow_types::output_dead_code::UnresolvedCatalogReferenceFinding],
        unused_catalog_entries: &[plow_types::output_dead_code::UnusedCatalogEntryFinding],
        empty_catalog_groups: &[plow_types::output_dead_code::EmptyCatalogGroupFinding],
    ) {
    ...
    fn add_unused_files(&mut self, items: &[plow_types::output_dead_code::UnusedFileFinding]) {
        for item in items {
            self.insert(format!(
                "unused-file:{}",
    ...
    fn add_unused_exports(&mut self, items: &[plow_types::output_dead_code::UnusedExportFinding]) {
        for item in items {
            self.insert(format!(
                "unused-export:{}:{}",
    ...
    fn add_unused_types(&mut self, items: &[plow_types::output_dead_code::UnusedTypeFinding]) {
        for item in items {
            self.insert(format!(
                "unused-type:{}:{}",
                relative_key_path(&item.export.path, self.root),
    ...
    fn add_private_type_leaks(
        &mut self,
        items: &[plow_types::output_dead_code::PrivateTypeLeakFinding],
    ) {
        for item in items {
            self.insert(format!(
                "private-type-leak:{}:{}:{}",
    ...
    fn add_invalid_client_exports(
        &mut self,
        items: &[plow_types::output_dead_code::InvalidClientExportFinding],
    ) {
        for item in items {
    ...
    fn add_mixed_client_server_barrels(
        &mut self,
        items: &[plow_types::output_dead_code::MixedClientServerBarrelFinding],
    ) {
        for item in items {
    ...
    fn add_misplaced_directives(
        &mut self,
        items: &[plow_types::output_dead_code::MisplacedDirectiveFinding],
    ) {
        for item in items {
    ...
    fn add_unprovided_injects(
        &mut self,
        items: &[plow_types::output_dead_code::UnprovidedInjectFinding],
    ) {
        for item in items {
    ...
    fn add_unrendered_components(
        &mut self,
        items: &[plow_types::output_dead_code::UnrenderedComponentFinding],
    ) {
        for item in items {
    ...
    fn add_unused_component_props(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedComponentPropFinding],
    ) {
        for item in items {
    ...
    fn add_unused_component_emits(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedComponentEmitFinding],
    ) {
        for item in items {
    ...
    fn add_unused_component_inputs(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedComponentInputFinding],
    ) {
        for item in items {
    ...
    fn add_unused_component_outputs(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedComponentOutputFinding],
    ) {
        for item in items {
    ...
    fn add_unused_svelte_events(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedSvelteEventFinding],
    ) {
        for item in items {
    ...
    fn add_unused_server_actions(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedServerActionFinding],
    ) {
        for item in items {
    ...
    fn add_unused_load_data_keys(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedLoadDataKeyFinding],
    ) {
        for item in items {
    ...
    fn add_route_collisions(
        &mut self,
        items: &[plow_types::output_dead_code::RouteCollisionFinding],
    ) {
        for item in items {
    ...
    fn add_dynamic_segment_name_conflicts(
        &mut self,
        items: &[plow_types::output_dead_code::DynamicSegmentNameConflictFinding],
    ) {
        for item in items {
    ...
    fn add_unused_dependencies(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedDependencyFinding],
    ) {
        for item in items {
    ...
    fn add_unused_dev_dependencies(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedDevDependencyFinding],
    ) {
        for item in items {
            self.insert(unused_dependency_key(&item.dep, self.root));
        }
    ...
    fn add_unused_optional_dependencies(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedOptionalDependencyFinding],
    ) {
        for item in items {
            self.insert(unused_dependency_key(&item.dep, self.root));
        }
    ...
    fn add_unused_enum_members(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedEnumMemberFinding],
    ) {
        for item in items {
    ...
    fn add_unused_class_members(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedClassMemberFinding],
    ) {
        for item in items {
            self.insert(unused_member_key(
                "unused-class-member",
    ...
    fn add_unused_store_members(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedStoreMemberFinding],
    ) {
        for item in items {
            self.insert(unused_member_key(
                "unused-store-member",
    ...
    fn add_unresolved_imports(
        &mut self,
        items: &[plow_types::output_dead_code::UnresolvedImportFinding],
    ) {
        for item in items {
            self.insert(format!(
                "unresolved-import:{}:{}",
    ...
    fn add_unlisted_dependencies(
        &mut self,
        items: &[plow_types::output_dead_code::UnlistedDependencyFinding],
    ) {
        for item in items {
    ...
    fn add_type_only_dependencies(
        &mut self,
        items: &[plow_types::output_dead_code::TypeOnlyDependencyFinding],
    ) {
        for item in items {
            self.insert(format!(
    ...
    fn add_test_only_dependencies(
        &mut self,
        items: &[plow_types::output_dead_code::TestOnlyDependencyFinding],
    ) {
        for item in items {
            self.insert(format!(
    ...
    fn add_stale_suppressions(&mut self, items: &[plow_types::results::StaleSuppression]) {
        for item in items {
    ...
    fn add_unresolved_catalog_references(
        &mut self,
        items: &[plow_types::output_dead_code::UnresolvedCatalogReferenceFinding],
    ) {
        for item in items {
    ...
    fn add_unused_catalog_entries(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedCatalogEntryFinding],
    ) {
        for item in items {
    ...
    fn add_empty_catalog_groups(
        &mut self,
        items: &[plow_types::output_dead_code::EmptyCatalogGroupFinding],
    ) {
        for item in items {
    ...
    fn add_unused_dependency_overrides(
        &mut self,
        items: &[plow_types::output_dead_code::UnusedDependencyOverrideFinding],
    ) {
        for item in items {
    ...
    fn add_misconfigured_dependency_overrides(
        &mut self,
        items: &[plow_types::output_dead_code::MisconfiguredDependencyOverrideFinding],
    ) {
        for item in items {
...
#[expect(
    ...
    reason = "plow standardizes on FxHashSet across audit attribution keys"
)]
...
fn classify_introduced_dead_code_fields(results: &plow_types::results::AnalysisResults) {
    let plow_types::results::AnalysisResults {
...
fn retain_introduced_fast_paths(
    unused_files: &mut Vec<plow_types::output_dead_code::UnusedFileFinding>,
    unused_exports: &mut Vec<plow_types::output_dead_code::UnusedExportFinding>,
    unused_types: &mut Vec<plow_types::output_dead_code::UnusedTypeFinding>,
    root: &Path,
    base: &FxHashSet<String>,
) {
...
pub fn health_keys(report: &plow_output::HealthReport, root: &Path) -> FxHashSet<String> {
    report
        .findings
        .iter()
...
pub fn health_finding_key(finding: &plow_output::ComplexityViolation, root: &Path) -> String {
    format!(
        "complexity:{}:{}:{:?}",
...
pub fn dupe_group_key(group: &plow_types::duplicates::CloneGroup, root: &Path) -> String {
...
mod tests {
    ...
    use plow_types::duplicates::{CloneGroup, CloneInstance, DuplicationReport};
    ...
    fn dead_code_keys_cover_graph_boundary_catalog_and_override_variants() {
        ...
        assert!(keys.contains(
            "missing-suppression-reason:src/app.ts:// plow-ignore-next-line unused-export"
        ));
        assert!(
    ...
    fn make_duplication_report(groups: Vec<CloneGroup>) -> DuplicationReport {
        DuplicationReport {
            ...
            stats: plow_types::duplicates::DuplicationStats::default(),
        }
    }
