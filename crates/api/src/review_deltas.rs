...
//!    (`plow_engine::public_api_package_entry_points`).
//!
...
use plow_types::output_dead_code::{BoundaryViolationFinding, CircularDependencyFinding};
...
/// `plow_engine::public_api_package_entry_points` (the R4 exports-aware
...
pub fn public_export_keys_for(
    ...
    root_pkg: Option<&plow_config::PackageJson>,
    workspaces: &[plow_config::WorkspaceInfo],
    root: &Path,
) -> FxHashSet<String> {
    let public_entries =
        plow_engine::public_api_package_entry_points(graph, config, root_pkg, workspaces);
...
mod tests {
    use super::*;
    use plow_types::output_dead_code::{BoundaryViolationFinding, CircularDependencyFinding};
    use plow_types::results::{BoundaryViolation, CircularDependency};
