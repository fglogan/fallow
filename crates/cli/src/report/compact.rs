...
pub(super) fn print_compact(results: &AnalysisResults, root: &Path) {
    print_lines(plow_api::build_compact_lines(results, root));
}
...
pub(super) fn print_grouped_compact(groups: &[ResultGroup], root: &Path) {
    print_lines(plow_api::build_grouped_compact_lines(groups, root));
}

pub(super) fn print_health_compact(report: &plow_output::HealthReport, root: &Path) {
    print_lines(plow_api::build_health_compact_lines(report, root));
}

pub(super) fn print_duplication_compact(report: &DuplicationReport, root: &Path) {
    print_lines(plow_api::build_duplication_compact_lines(report, root));
}
