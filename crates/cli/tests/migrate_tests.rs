...
// ---------------------------------------------------------------------------
// Glob roundtrip: migrate -> plow list --files, asserting that plow's
// discovered file set matches the set knip's `ignore` globs would exclude
// from analysis. Catches drift between knip's glob engine and plow's
...
//
// Note: plow's source discovery walks every supported source file under
...
// drift. Knip's `entry` and plow's `entry` carry the same semantics
...
fn roundtrip_fixture(suffix: &str) -> std::path::PathBuf {
    ...
    // Files that should NOT appear in plow's discovered set. Two
    // categories:
    ...
    //   (2) excluded by plow's built-in defaults regardless of migration:
...
fn migrate_roundtrip_globs_match_knip_documented_semantics() {
    ...
    // Step 2: run plow list --files against the migrated config. This is
    // plow's source-discovery surface: it returns precisely the set of
    ...
    // knip's documented glob semantics. If knip and plow's globset
    // diverge on any of these patterns, this assertion fails loudly. The
    // list is sorted because `plow list --files` returns sorted output.
