...
//! child `plow` binary with `PLOW_TEST_SIGNAL_HELPER=1`, which goes
...
//! (SIGINT or SIGTERM) to the child plow via `kill`, then asserts:
//!  - The plow exit code matches POSIX convention (130 / 143).
...
//! separate plow child means each test gets its own PID and its own
//! signal disposition.
//!
...
/// Spawn a child `plow` binary in signal-helper mode and read the
...
///
/// Killing + waiting the spawned plow on PID-read failure is the
...
fn spawn_signal_helper_with(graceful: bool) -> (std::process::Child, u32) {
    let plow_bin = env!("CARGO_BIN_EXE_plow");
    let mut builder = Command::new(plow_bin);
    builder
        .env("PLOW_TEST_SIGNAL_HELPER", "1")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    if graceful {
        builder.env("PLOW_TEST_SIGNAL_HELPER_GRACEFUL", "1");
    }
    let mut child = builder.spawn().expect("spawn plow signal helper");
...
fn sigint_in_graceful_mode_drains_children_but_does_not_exit() {
    let (mut plow, sleep_pid) = spawn_signal_helper_with(true);
    assert!(
        !wait_for_pid_dead(sleep_pid, Duration::from_millis(100)),
        "inner sleep PID {sleep_pid} should be alive after helper start"
    );
