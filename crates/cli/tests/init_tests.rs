...
fn init_creates_plowrc_json() {
    ...
    let output = run_plow_raw(&["init", "--root", dir.to_str().unwrap(), "--quiet"]);
    assert_eq!(
        output.code, 0,
    ...
    assert!(
        ...
        "init should create .plowrc.json"
    );
    cleanup(&dir);
}
...
fn init_creates_toml_with_flag() {
    ...
    let output = run_plow_raw(&["init", "--toml", "--root", dir.to_str().unwrap(), "--quiet"]);
    assert_eq!(
        output.code, 0,
    ...
    assert!(
        dir.join("plow.toml").exists(),
        "init --toml should create plow.toml"
    );
    cleanup(&dir);
}
...
fn init_exits_nonzero_if_config_exists() {
    ...
    run_plow_raw(&["init", "--root", dir.to_str().unwrap(), "--quiet"]);
    let output = run_plow_raw(&["init", "--root", dir.to_str().unwrap(), "--quiet"]);
    assert_ne!(
        output.code, 0,
...
fn init_created_config_is_valid_json() {
    ...
    run_plow_raw(&["init", "--root", dir.to_str().unwrap(), "--quiet"]);
    let content = fs::read_to_string(dir.join(".plowrc.json")).unwrap();
...
fn hooks_namespace_installs_and_uninstalls_git_hook() {
    ...
    assert!(hook.contains("Generated by plow hooks install --target git"));
    assert!(hook.contains("BASE=\"develop\""));

    let uninstall = run_plow_raw(&["--root", root, "hooks", "uninstall", "--target", "git"]);
    assert_eq!(
        uninstall.code, 0,
...
fn hooks_namespace_agent_dry_run_uses_setup_hooks_engine() {
    ...
    assert!(
        output
            .stderr
            .contains("plow hooks install --target agent (install) (dry run)"),
