...
(if $vitals.maintainability_avg then
  "[Maintainability](https://docs.genesis-plow.dev/explanations/health): **\($vitals.maintainability_avg | . * 10 | round / 10)** / 100" +
...
(if $filtered > 0 and $inline > 0 then
  "**\($inline)** inline comments on your changes \u00b7 \($filtered) findings in files not changed in this PR \u2014 run `plow dead-code` locally to see them\n\n"
