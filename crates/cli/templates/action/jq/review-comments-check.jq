...
def footer(rule): "\n\n---\n<sub><a href=\"https://docs.genesis-plow.dev/explanations/dead-code#" + rule + "\">Docs</a> \u00b7 Disagree? <a href=\"https://docs.genesis-plow.dev/configuration/suppression\">Configure or suppress</a></sub>";
...
[
  ...
  (.unused_dependencies[]? | {
    ...
    body: ":warning: **Unused dependency**\n\n`\(.package_name)` is listed in `\(.location)` but no file imports it directly.\(workspace_context)\(dependency_action(.package_name))\n\n<details>\n<summary>Why this matters</summary>\n\nUnused dependencies slow down installs, inflate `node_modules`, and add noise to security audits.\n</details>\n\n> Some packages are used indirectly (peer dependencies, framework internals, or plugin systems). If that\u2019s the case, add it to [`ignoreDependencies`](https://docs.genesis-plow.dev/configuration/overview) in `.plowrc.json`.\(footer("unused-dependencies"))"
  }),
  (.unused_dev_dependencies[]? | {
    type: "other",
    path: (prefix + .path),
    line: (if .line > 0 then .line else 1 end),
    body: ":warning: **Unused devDependency**\n\n`\(.package_name)` is listed in `devDependencies` but no file imports it in this workspace.\(workspace_context)\(dependency_action(.package_name))\n\n> Used by a tool that doesn\u2019t import directly (e.g., a CLI, plugin, or preset)? Add it to [`ignoreDependencies`](https://docs.genesis-plow.dev/configuration/overview) in `.plowrc.json`.\(footer("unused-dependencies"))"
  }),
  ...
  (.stale_suppressions[]? | {
    ...
    body: (if .origin.type == "jsdoc_tag" then
      ...
      ":broom: **Stale suppression**\n\nThis `\(if .origin.is_file_level then "plow-ignore-file" else "plow-ignore-next-line" end)` comment\(if .origin.issue_kind then " for `\(.origin.issue_kind)`" else "" end) no longer matches any active issue.\n\n**Action:** Remove the suppression comment to keep the codebase clean.\(footer("stale-suppressions"))"
    end)
  })
] | .[:($ENV.MAX | tonumber)]
