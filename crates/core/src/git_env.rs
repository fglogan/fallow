//! Helpers for invoking `git` from plow without inheriting ambient repo
//! state from the parent process.
//!
//! When plow is invoked from a git hook (`pre-commit`, `pre-push`,
...
//! during `git commit`). When plow then spawns its own `git` subprocess from
...
//! call fails.
//!
//! Plow always operates against the repository at `--root` (or the cwd) and
...
//! `GIT_EXEC_PATH`. Those are either harmless to plow's read-only git
//! invocations or required for plow's tests that depend on the parent shell's
//! git config.
...
/// paths relative to the parent's cwd, and they break plow's git invocations
/// when plow runs from a different cwd.
...
///
/// Apply to every `git` subprocess plow spawns from production code. The
