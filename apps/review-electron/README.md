# Plow Review (Electron)

A native desktop app for reviewing code changes (especially agent-authored ones),
grounded in Plow's deterministic engine and fed back to a coding agent.

It turns `plow review` output into a guided walkthrough (Review Focus, ordered
...
Set `PLOW_BIN` to a `plow` binary if it is not on `PATH`:

```bash
PLOW_BIN=../../target/release/plow npm run dev
```
...
- **main/** Electron main process.
  - `review.ts` spawns `plow review --format json` (and `--walkthrough-guide` /
  ...
  - `feed.ts` the `.plow-review/feed.jsonl` agent feed.
- **preload/** `contextBridge` exposing a typed `window.plow` API.
- **model/** surface-agnostic types + the pure `toWalkthroughDocument` adapter
  (drops decisions lacking a Plow `signal_id`: anti-hallucination).
...
- **inspector/** the grounded inspector:
  - `babelInspectorSource.ts` stamps root-relative `data-plow-source` on JSX in
    dev (React-19-safe; same path-space as `plow review` for a correct join).
...
`plow review --format json` -> `toWalkthroughDocument` -> renderer walkthrough.
Picker click -> `data-plow-source` -> bridge -> `buildInspectorCard` (facts from
the latest review) -> `inspect:selection` -> InspectorCard. Annotations + selections
-> `.plow-review/feed.jsonl`; signal_id-anchored judgments validate via
`plow review --walkthrough-file` (the verifier is the graph, not a second model).
