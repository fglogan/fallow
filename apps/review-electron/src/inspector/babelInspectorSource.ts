...
/** `root`: the review/project root, so stamped paths match Plow's workspace-relative paths. */
type Options = { root?: string };

/**
 * Babel plugin: stamp `data-plow-source="file:line:col"` on every JSX element
 ...
 * as `plow review` output, which is what lets the inspector JOIN a selection to
 * grounded facts.
 */
export const babelInspectorSource = ({ types: t }: Api, options: Options = {}): PluginObj => ({
  name: "plow-inspector-source",
  visitor: {
    JSXOpeningElement(path, state: PluginPass) {
      ...
      const already = path.node.attributes.some(
        (attr) =>
          ...
          attr.name.name === "data-plow-source",
      );
      if (already) return;
      ...
      path.node.attributes.push(
        t.jsxAttribute(t.jsxIdentifier("data-plow-source"), t.stringLiteral(value)),
      );
    },
  },
});
