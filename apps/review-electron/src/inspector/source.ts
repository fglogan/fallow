/** A source location stamped onto a DOM element by the Plow inspector plugin. */
...
/** Walk up from `el` to the nearest element carrying `data-plow-source`. */
export const readSourceFromElement = (el: SourceElement): Source | null => {
  const node = el.closest("[data-plow-source]");
  const value = node?.getAttribute("data-plow-source") ?? null;
