...
Environment: Apple M5 (10 cores), 32 GB RAM, macOS 25.3.0, Node v22.21.1, rustc 1.93.0. plow 1.2.0, knip 5.87.0, knip 6.0.0, jscpd 4.0.8, madge 8.0.0. Median of 5 runs, 2 warmup.

### Dead code: plow dead-code vs knip

| Project | Files | plow | knip v5 | knip v6 | vs v5 | vs v6 | plow RSS | knip v5 RSS | knip v6 RSS |
|:--------|------:|-------:|--------:|--------:|------:|------:|-----------:|------------:|------------:|
...
| Project | Files | plow | madge | dpdm | vs madge | vs dpdm | plow RSS |
|:--------|------:|-------:|------:|-----:|---------:|--------:|-----------:|
...
Note: plow runs a full analysis pipeline (discovery, parsing, graph building, SCC detection) while madge/dpdm only build a dependency graph from imports. On large monorepos the pipeline overhead dominates. On small-to-medium projects plow's native speed wins. Madge and dpdm report `?` for cycle counts on many projects, suggesting incomplete detection.

### Summary ranges

| Comparison | Speed | Memory |
|:-----------|:------|:-------|
| plow vs knip v5 | 6-46x faster | 4-11x less |
| plow vs knip v6 | 3-18x faster | 3-6x less |
| plow vs jscpd | 8-34x faster | 2-4x less |
| plow vs madge | 1-32x faster (small-medium), 0.3-0.5x on large monorepos | 4-13x less |
