# Plow domain language

Shared glossary for everyone working on plow. Keeps the language consistent across code, docs, issues, commits, and agent prompts. Update this file whenever a new term enters the vocabulary or an existing term gets reused for a second concept.
...
**Suppress**
The mechanism to silence an Issue at a specific location. Two forms: `// plow-ignore-next-line <issue-type>` (single line) and `// plow-ignore-file <issue-type>` (whole file). Distinct from `rules` config which is project-wide.
_Avoid_: "ignore", "skip", "exclude" (those have other meanings in config schemas).

## Analysis areas

The five top-level analysis areas the bare `plow` command produces. Plow is a codebase analyzer, not a dead-code tool; references to plow MUST cite multiple areas.
...
Each subcommand owns a piece of the analysis pipeline. Bare `plow` runs the full set; named subcommands narrow the scope.
...
_Avoid_: calling this "plow check" or "plow dead-code"; both are scoped subcommands.

**`plow dead-code`** (alias: `plow check`)
Dead-code-only analysis. Faster than bare `plow`. Does not run dupes or health. Canonical name is `dead-code`; `check` is retained as a backwards-compatible alias from before the 2026-03-26 rename (commit 75c9884e0e3).
...
**`plow init`**, **`plow validate`**, **`plow schema`**, **`plow migrate`**
Config bootstrap, validation, schema emission, and migration from knip / jscpd configs.

## Plugin and workspace concepts

**Plugin**
A plow framework-detection plugin. Knows about a specific framework or build tool (Next.js, Vite, Astro, Storybook, Vitest, etc.) and contributes entry points, file patterns, suppress rules, and dependency knowledge. Lives in `crates/core/src/plugins/` and `npm/plow/skills/` per plugin discovery rules.
_Avoid_: "plow plugin" when referring to a Claude Code plugin or an npm/cargo plugin; qualify the context every time.

**Workspace**
A pnpm/npm/yarn monorepo workspace. Plow discovers workspaces via package.json globs and analyzes each workspace package's entry points and boundaries.
_Avoid_: using "workspace" for the broader analysis project; the project as a whole is the project root.

**Entry point**
A file plow treats as a graph root: not unused even if nothing else imports it. Determined by config (`entry`), plugins (framework conventions), and package.json (`bin`, `main`, `module`, `exports`).

**Re-export chain**
A barrel-file propagation path. When `b.ts` re-exports `a.ts` and `c.ts` re-exports `b.ts`, plow resolves the chain iteratively (ADR-005) so that usage of the symbol in `c.ts` counts against the original definition in `a.ts`.
...
**Hotspot**
A complexity hotspot: function or module above a complexity threshold. Surfaced via `plow health --hotspots`.

**CRAP**
Change Risk Anti-Patterns score. Combines cyclomatic complexity with coverage to flag risky-untested code. Plow ships three CRAP tiers depending on what coverage signal is available.

**Tier (CRAP)**
The data quality of the CRAP score: `estimated` (no coverage data, complexity only), `Istanbul` (Istanbul/c8 coverage JSON), or `binary` (plow runtime coverage from the sidecar). Higher tier = stronger signal.
_Avoid_: using "tier" for license plans without qualifying as "license tier".

**Risk profile**
A composed health view per-module: complexity, ownership risk (bus factor, drift, declared owner), coupling concentration, duplication penalty, large-function drill-down. Surfaced via `plow health` with risk-profile flags.

**Ownership risk**
Bus factor + author drift + declared owner mismatch + unowned hotspots. Surfaced via `plow health --hotspots --ownership`. Based on git blame, not LSP semantics.

**Signals**
The `signals[]` array in `plow audit` JSON output. Each signal is one piece of evidence (verdict, line overlap, hot-path-touched, etc.) used by the audit consumer (action, CI, MR comment).
...
**Sidecar**
The closed-source binary that captures V8 runtime coverage and emits protocol-compliant payloads. Distributed via the `plow-cov` npm package. First clean release was `sidecar-v0.1.5`; v0.1.0 to v0.1.4 are tombstones.

**Protocol**
The `plow-cov-protocol` crate. Open-source schema that the sidecar and plow agree on for runtime coverage payloads. Lives in a separate repo, published to crates.io.
_Avoid_: "protocol" without qualifying when the conversation drifts to other RPC concepts.

**Trial**
A time-limited license that unlocks paid features. Lives behind the `plow license` command. Trials feed the grace ladder.
...
Plow emits the same Issue set in multiple formats, selected via `--format`. Format is CLI-only, not in config.
...
**Config file**
First-match-wins discovery: `.plowrc.json` > `.plowrc.jsonc` > `plow.toml` > `.plow.toml`. No merging. The first one found wins.
_Avoid_: ESLint terminology like `root: true`; plow's model is different.
...
**Companion repo**
A peer repository to `fglogan/genesis-plow` that ships a related artifact: `plow-docs` (Mintlify docs), `plow-skills` (the public `/plow` Claude Code skill), `plow-cov-protocol` (open-source protocol crate), `plow-tools` (marketing site), `plow-cloud` (private cloud monorepo).

**Worktree**
A parallel git worktree of plow main. The bare-config'd `~/Sites/plow/` is the primary checkout; `~/Sites/plow-2/` is a sister worktree. Multiple agents and background sessions often run across these in parallel.

**Bare repo**
The bare-config'd `~/Sites/plow/` checkout where commits originate. Worktrees and parallel sessions must not push from their own working dirs without an ephemeral worktree (see `CLAUDE.md` worktree rules).
...
**.plans/**
Gitignored directory for in-flight feature plans. `/plow-implement` writes its Phase 1 plan to `.plans/<feature-slug>.md`; downstream skills (`/panel-review`, the implementation phase, `/plow-review`) re-read the file rather than relying on context overflow. Plans are transient; promoted concepts go to ADRs.
...
- **Signals** belong to one `plow audit` run and are emitted to one audit consumer (action, CI, MR comment).
...
- **Plugin**: plow framework-detection plugin vs Claude Code plugin vs generic npm/cargo plugin. Default in this codebase is plow plugin; qualify otherwise.
- **Coverage**: runtime coverage (paid sidecar feature) vs `plow coverage` command surface vs test coverage. Use the qualified form when discussing across two of these.
...
- **Check**: legacy alias for `plow dead-code` (kept for backwards compatibility) vs general code check. New code, docs, and skills should use `dead-code`.
- **Health**: `plow health` command vs general code health. Same as check.
- **Family**: duplicate clone family (this glossary) vs plugin family (does not exist as a plow concept; treat as a typo if encountered).
- **Audit**: `plow audit` command vs general code audit (e.g. SIG audit, security audit). Qualify by command name when ambiguous.
- **Protocol**: `plow-cov-protocol` crate vs general RPC protocol terminology. Default to the crate in plow contexts.
- **Bare**: `bare plow` (no subcommand, full pipeline) vs bare git repo (the `~/Sites/plow/` checkout). Distinct concepts that both appear in dev conversations.
...
- "Dead code tool" as a category for plow has been retired. Plow is a **codebase analyzer**; never reduce to dead-code-only language in public communication.
- "Backlog" is not a plow term. Issues live in GitHub Issues at `fglogan/genesis-plow`; do not call that "the backlog".
