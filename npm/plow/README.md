...
Static analysis is free and open source. An optional paid runtime layer (Plow Runtime) adds production execution evidence. Rust-native, sub-second, 123 framework plugins, no Node.js runtime dependency for analysis. plow's edge is doing more in one pass: dead code, duplication, dependencies, complexity, architecture, and security in a single tool. For head-to-head timings against [knip](https://knip.dev) and [jscpd](https://github.com/kucherenko/jscpd), see the [benchmarks](https://github.com/fglogan/genesis-plow/tree/main/benchmarks): plow is faster than knip on smaller projects, knip is faster on several larger repos, and jscpd's Rust rewrite is faster for raw duplication scanning.
...
Agents that speak MCP can launch the bundled `plow-mcp` server. Installed as a devDependency, the binary lives in `node_modules/.bin/` and is not on your `PATH`, so launch it through your package manager's runner:
...
Swap `npx` for `pnpm exec` / `yarn` / `bunx` to match your package manager. If `plow-mcp` is installed globally (on your `PATH`), `"command": "plow-mcp"` works directly. See the [MCP integration guide](https://docs.genesis-plow.dev/integrations/mcp).
