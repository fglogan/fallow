...
Complete command and flag specifications for all plow CLI commands.
...
Analyzes the project for unused files, exports, dependencies, types, members, and more. Running `plow` with no subcommand runs all analyses (dead code + duplication + complexity). Use `plow dead-code` for dead code only.
...
# Only unused exports
plow dead-code --format json --quiet --unused-exports
...
plow dead-code --format json --quiet --workspace web,admin

# Glob (matches package name OR relative path)
plow dead-code --format json --quiet --workspace 'apps/*'

# Exclude a workspace from the set
plow dead-code --format json --quiet --workspace 'apps/*,!apps/legacy'

# Monorepo CI: auto-scope to workspaces containing any file changed since origin/main
plow dead-code --format json --quiet --changed-workspaces origin/main
...
plow dead-code --format json --quiet --save-regression-baseline
plow dead-code --format json --quiet --fail-on-regression --tolerance 2%

# Scope to specific files (e.g., lint-staged)
plow dead-code --format json --quiet --file src/utils.ts --file src/helpers.ts

# Catch typos in entry file exports
plow dead-code --format json --quiet --include-entry-exports
```
...
By default, `plow dupes` skips generated framework output matching `**/.next/**`, `**/.nuxt/**`, `**/.svelte-kit/**`, `**/.turbo/**`, `**/.parcel-cache/**`, `**/.vite/**`, `**/.cache/**`, `**/out/**`, and `**/storybook-static/**`. These defaults merge with `duplicates.ignore`. Set `duplicates.ignoreDefaults = false` to opt out and use only your configured ignore list. If the reported duplication percentage drops after upgrading, this generated-output filtering is the expected reason.
...
| `--min-occurrences` | `string` | - | Minimum number of occurrences before a clone group is reported (must be ≥ 2). Raise to skip pair-only clones and focus on widespread copy-paste worth refactoring. `plow init` writes `minOccurrences: 3` into new projects. |
...
plow dupes --format json --quiet --mode semantic

# Cross-directory only, fail at 5%
plow dupes --format json --quiet --skip-local --threshold 5

# Trace clones at a specific location
plow dupes --format json --quiet --trace src/utils.ts:42

# Deep-dive a clone group by its dup:<id> fingerprint (from the listing or JSON)
plow dupes --format json --quiet --trace dup:7f3a2c1e

# Only check duplication in changed files
plow dupes --format json --quiet --changed-since main
...
| `--no-create-config` | `bool` | `false` | Refuse to create a new `.plowrc.json` when none exists. The duplicate-export config-add path is skipped with `skip_reason: "no_create_config"`; source-file edits proceed normally. Use in pre-commit hooks, CI bots, and `plow watch` where silently materialising a new top-level file would surprise the user. |
...
- **Files with an unresolved import.** The file itself imports something plow could not resolve, so its local usage graph is incomplete.

JSON output carries `{"type": "skipped", "path": "...", "skipped": true, "skip_reason": "low_confidence_off_graph"}` (or `"low_confidence_unresolved_imports"`) plus a top-level counter `skipped_low_confidence_exports: number` (always present), disjoint from `skipped`. Unlike the drift and encoding skips this is INTENTIONAL and does NOT change the exit code; the export stays reported by `plow dead-code` for manual review. High-confidence exports in normal source files are removed unchanged. The AI agent should report kept exports to the user and let them decide whether the export is truly unused before removing it by hand.

### File encoding contract

`plow fix` is UTF-8 only. Two encoding shapes that previously caused silent corruption are handled explicitly (issue #475):

- **UTF-8 BOM round-trip.** Files with a leading UTF-8 byte-order mark (`EF BB BF`, common on Windows-authored TypeScript) are read with the BOM stripped before line-offset computation and parsing, so reported line numbers do not shift by the BOM codepoint, and the BOM is re-prepended on write so the file's encoding is preserved on round-trip. plow neither adds nor removes a BOM; if your input has one, the output has one.

- **Mixed CRLF / LF rejection.** Files containing both `\r\n` and bare-LF line endings (common after cross-platform edits without `core.autocrlf`) are skipped instead of silently rewritten to the wrong offsets. The stderr message names the remediation: `Skipping <path>: file has mixed CRLF/LF line endings. Normalize with dos2unix or set git config core.autocrlf input, then re-run plow fix.`. JSON output carries `{"type": "skipped", "path": "...", "skipped": true, "skip_reason": "mixed_line_endings"}` plus a top-level counter `skipped_mixed_line_endings: number` (always present) disjoint from `skipped_content_changed`. Any non-zero mixed-EOL count exits the run with code 2.

  **The skip is NOT self-healing**. Re-running `plow fix` produces the same skip; the AI agent or user must run `dos2unix <path>` (or set `git config core.autocrlf input` and re-checkout) before plow can act on the file. When the same file carries findings for multiple fixers (e.g. an unused export AND an unused enum member), the skip is reported once per file, not once per fixer.
...
| `--workspaces` | `bool` | `false` | Show discovered monorepo workspaces plus any workspace-discovery diagnostics (malformed `package.json`, unreachable glob matches, missing tsconfig references). Available as the `plow workspaces` alias too. |
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--toml` | `bool` | `false` | Create `plow.toml` instead of `.plowrc.json` |
...
| `--hooks` | `bool` | `false` | Scaffold a pre-commit git hook that runs `plow audit --base <ref> --quiet --gate-marker pre-commit`. Alias for `plow hooks install --target git` |
...
```bash
plow init              # creates .plowrc.json with $schema
plow init --toml       # creates plow.toml
plow init --agents     # scaffolds a starter AGENTS.md prefilled from detected project info (never overwrites)
plow hooks install --target git
plow hooks install --target git --branch develop  # fallback base branch when no upstream is set
```

## `hooks`: Managed Hook Status And Installation

```bash
plow hooks status --format json
plow hooks install --target git
plow hooks install --target agent
plow hooks uninstall --target git
plow hooks uninstall --target agent
```

`hooks status` is read-only and reports `git`, `claude`, and `codex` surfaces. Each surface includes `installed`, `managed_block_present`, `user_edited`, and `path`; generated agent scripts also include `script_version` and `min_version_floor`. Use it before mutating setup so agents can distinguish plow-managed artifacts from user-owned hooks or partial managed blocks.

---

## `migrate`: Config Migration

Migrates configuration from knip and/or jscpd to plow. Auto-detects config files.
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--toml` | `bool` | `false` | Output as `plow.toml` (mutually exclusive with `--jsonc`) |
| `--jsonc` | `bool` | `false` | Write to `.plowrc.jsonc` instead of `.plowrc.json`. Same JSONC content either way; the `.jsonc` extension lets editors auto-detect JSON-with-comments syntax highlighting |
| `--dry-run` | `bool` | `false` | Preview without writing |
| `--from` | `string` | - | Specify source config file path |

Common global flags for this command: [`--root`](#global-flags), [`--config`](#global-flags).
<!-- generated:flags:migrate:end -->
Without `--jsonc` or `--toml`, plow auto-mirrors the source extension: a `knip.jsonc` migration writes `.plowrc.jsonc`, a `knip.json` migration writes `.plowrc.json`.
...
```bash
plow migrate --dry-run        # preview
plow migrate                  # auto-detect; mirrors source extension
plow migrate --jsonc          # force .plowrc.jsonc output
plow migrate --toml           # output as plow.toml
plow migrate --from knip.jsonc
```
...
| `--max-crap` | `string` | - | Fail if any function has CRAP score >= threshold. CRAP combines complexity with coverage (`CC^2 * (1 - cov/100)^3 + CC`). Pair with `--coverage` for accurate per-function CRAP; without Istanbul data plow estimates coverage from the module graph. |
...
| `--trend` | `bool` | `false` | Compare current metrics against the most recent saved snapshot. Reads from `.plow/snapshots/` and shows per-metric deltas with directional indicators (improving/declining/stable). Implies `--score`. |
| `--coverage` | `string` | - | Path to Istanbul-format coverage data (`coverage-final.json`) for accurate per-function CRAP scores. Uses `CC^2 * (1-cov/100)^3 + CC` instead of static binary model. Relative paths resolve against `--root`. Falls back to `PLOW_COVERAGE`, then `health.coverage`, then auto-detection. |
| `--coverage-root` | `string` | - | Absolute prefix to strip from file paths in coverage data before prepending the project root. For CI/Docker environments where coverage was generated with different absolute paths. Falls back to `PLOW_COVERAGE_ROOT`, then `health.coverageRoot`. |
| `--runtime-coverage` | `string` | - | Merge runtime-coverage input into the health report. Accepts a V8 coverage directory (`NODE_V8_COVERAGE=...`), a single V8 coverage JSON file, or an Istanbul `coverage-final.json`. One local capture is free and does not require a license; continuous/cloud or multi-capture runtime monitoring requires an active license or trial (`plow license activate --trial --email <addr>`). JSON output gains a `runtime_coverage` object with a top-level report verdict, per-finding `verdict` (`safe_to_delete` / `review_required` / `low_traffic` / `coverage_unavailable` / `active`), a per-finding suppression `id` (`plow:prod:<hash>`, hashes the current line), an optional cross-surface `stable_id` join key (`plow:fn:<hash>`, hashes file + name + start line; one value per function across findings / hot-paths / blast-radius / importance and across V8/Istanbul/oxc producers), an optional content-digest `source_hash` (line-move-immune, so baselines survive a pure line shift), an evidence block, and percentile-ranked hot paths. On protocol-0.3+ sidecars the `summary` also carries an optional `capture_quality` block (`window_seconds`, `instances_observed`, `lazy_parse_warning`, `untracked_ratio_percent`) that flags short-window captures where lazy-parsed scripts may not appear. |
...
The gate flag in play determines what drives the exit code. Plain `plow health` (no gate flag) stays advisory but still fails on any finding (back-compat).

| Invocation | Exit 0 when | Exit 1 when |
|------------|-------------|-------------|
| `plow health` (no gate flag) | no function exceeds a threshold | any function exceeds a threshold |
...
`--report-only` with `--min-score` / `--min-severity` exits 2 (mutually exclusive). The `--runtime-coverage` and coverage-gap gates stay independent and are not demoted by `--min-score`. For gating only newly-introduced complexity, use `plow audit --gate new-only`.

### Examples

```bash
# Full complexity analysis with JSON output
plow health --format json --quiet

# Project health score with letter grade
plow health --format json --quiet --score

# CI gate: fail if score below 70
plow health --format json --quiet --min-score 70

# Top 10 most complex functions
plow health --format json --quiet --top 10

# Sort by cognitive complexity
plow health --format json --quiet --sort cognitive

# Custom thresholds
plow health --format json --quiet --max-cyclomatic 15 --max-cognitive 10

# Per-file health scores
plow health --format json --quiet --file-scores

# Top 20 files by triage concern
plow health --format json --quiet --file-scores --top 20

# Only analyze files changed since main
plow health --format json --quiet --changed-since main

# Single workspace package
plow health --format json --quiet --workspace my-package

# Incremental adoption with baseline
plow health --format json --quiet --save-baseline plow-baselines/health.json
plow health --format json --quiet --baseline plow-baselines/health.json

# CI: fail if any function is too complex
plow health --max-cyclomatic 25 --max-cognitive 20 --quiet

# Hotspot analysis (complex + frequently changing files)
plow health --format json --quiet --hotspots

# Hotspots from the last year
plow health --format json --quiet --hotspots --since 1y

# Hotspots with at least 5 commits
plow health --format json --quiet --hotspots --min-commits 5

# Top 10 hotspots from the last 90 days
plow health --format json --quiet --hotspots --since 90d --top 10

# Ranked refactoring recommendations
plow health --format json --quiet --targets

# Top 5 refactoring targets
plow health --format json --quiet --targets --top 5

# Only low-effort refactoring targets (quick wins)
plow health --format json --quiet --effort low

# Save a vital signs snapshot for trend tracking
plow health --format json --quiet --save-snapshot

# Save snapshot to a custom path
plow health --format json --quiet --save-snapshot .plow/baseline-snapshot.json

# Compare current metrics against the most recent snapshot
plow health --format json --quiet --trend
```
...
Metrics tracked: `score`, `dead_file_pct`, `dead_export_pct`, `avg_cyclomatic`, `maintainability_avg`, `unused_dep_count`, `circular_dep_count`, `hotspot_count`, `unit_size_very_high_pct`, `p95_fan_in`, `duplication_pct`. Each metric includes `direction` (`improving`, `declining`, `stable`). Percentage metrics include `previous_count`/`current_count` with raw numerator/denominator. `--trend` requires at least one saved snapshot in `.plow/snapshots/`. When comparing against a snapshot from an older schema version (current: v8), the trend output warns that score deltas may reflect formula changes.
...
| `--dead-code-baseline` | `string` | - | Baseline file (produced by `plow dead-code --save-baseline`). Pre-existing dead-code issues are excluded from the verdict. |
| `--health-baseline` | `string` | - | Baseline file (produced by `plow health --save-baseline`). Pre-existing complexity findings are excluded from the verdict. |
| `--dupes-baseline` | `string` | - | Baseline file (produced by `plow dupes --save-baseline`). Pre-existing clone groups are excluded from the verdict. |
...
| `--coverage` | `string` | - | Path to Istanbul-format coverage data (`coverage-final.json`) for accurate per-function CRAP scores in the health sub-analysis. Same format and semantics as `health --coverage`. Also configurable via `PLOW_COVERAGE`. Relative paths resolve against `--root`. |
| `--coverage-root` | `string` | - | Absolute prefix to strip from file paths in coverage data before prepending the project root. Also configurable via `PLOW_COVERAGE_ROOT`. Use when coverage was generated under a different checkout root in CI / Docker (e.g., `/home/runner/work/myapp` on GitHub Actions). |
...
| `--runtime-coverage` | `string` | - | Paid runtime-coverage sidecar input. Accepts a V8 directory, a single V8 JSON file, or an Istanbul coverage map JSON. Spawns the `plow-cov` sidecar as part of the audit pipeline so the `hot-path-touched` verdict surfaces alongside dead-code and complexity findings without requiring a second `plow health` invocation in CI. License-gated; the verdict is informational (no exit code change) until a future `--gate hot-path-touched` knob lands |
...
| `--gate-marker` | `string` | - | Internal marker identifying a gate run (e.g. `pre-commit`), set by the generated git hook so Plow Impact can record a containment event when the gate blocks then clears. Hidden; never changes the verdict, exit code, or output |
| `--brief` | `bool` | `false` | Render the deterministic review brief instead of the gating audit report. The brief answers "where do I look?" rather than "will CI block this?", runs the same analysis, and ALWAYS exits 0 (the verdict is carried informationally). Implied by `plow review`. Orthogonal to `--format` |
...
| `--walkthrough-file` | `string` | - | Ingest an agent's judgment JSON and POST-VALIDATE it against the LIVE graph. Rejects any judgment whose `signal_id` plow did not emit (anti-hallucination); refuses the whole payload as stale when the echoed graph-snapshot hash no longer matches (the tree moved). The verifier is the graph, not a second model. Implies the brief; always exits 0. The agent's free-text framing is fenced as non-deterministic and never gates or auto-posts |
...
| `--mark-viewed` | `string` | - | Record one or more changed files as VIEWED in the local walkthrough viewed-state ledger (`.plow/walkthrough-state.json`), then render the tour. Files already viewed (and still current) collapse into the Cleared panel. Repeatable. Stale marks (the tree moved) are ignored on render but never deleted. Only consulted on the `--walkthrough` path |
...
plow audit --format json --quiet

# Explicit base ref
plow audit --format json --quiet --base main

# Audit last 3 commits
plow audit --format json --quiet --base HEAD~3

# Strict mode: fail on inherited findings too
plow audit --format json --quiet --gate all

# Production code only in a monorepo workspace
plow audit --format json --quiet --production --workspace @app/api

# Production-only health, full-tree dead-code and dupes
plow audit --format json --quiet --production-health --workspace @app/api
...
# Or set these under `audit.*Baseline` in .plowrc.json so `plow audit` picks them up with no flags.
...
plow flags --format json --quiet

# Top 10 flags
plow flags --format json --quiet --top 10

# Single workspace package
plow flags --format json --quiet --workspace my-package
```
...
The second rule family is a data-driven `tainted-sink` catalogue: syntactic dangerous-sink candidates across the catalogue categories listed below. Most rows require a non-literal argument; narrowly literal-aware rows flag deterministic unsafe literals such as wildcard `postMessage` origins, weak crypto algorithms, disabled TLS validation, and JWT algorithm issues. Plow prefers false-negatives over false-positives.
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--runtime-coverage` | `string` | - | Paid runtime-coverage sidecar input. Accepts a V8 directory, a single V8 JSON file, or an Istanbul coverage map JSON. When set, `plow security` annotates tainted-sink candidates with production runtime state and uses that state as an additive ranking signal |
| `--min-invocations-hot` | `string` | `100` | Threshold for hot-path classification, forwarded to the sidecar when `--runtime-coverage` is set |
...
```bash
plow security --format json --quiet
plow security --ci --sarif-file plow-security.sarif
git diff --unified=0 origin/main...HEAD | plow security --diff-file -
# Regression gate: fail (exit 8) only on candidates introduced in the changed lines
plow security --gate new --changed-since origin/main
git diff --cached --unified=0 | plow security --gate new --diff-stdin

# Reachability gate: fail when existing sinks become entry-point reachable
plow security --gate newly-reachable --changed-since origin/main
```
...
`plow security --summary --format json --quiet` emits the same `kind`, `schema_version`, `version`, `elapsed_ms`, and `config` metadata, but replaces candidate arrays with `summary` aggregate counts:
...
```bash
plow inspect --file src/api.ts --format json --quiet
plow inspect --symbol src/api.ts:fetchUser --format json --quiet
```
...
```bash
plow trace src/utils.ts:formatDate
plow trace src/utils.ts:formatDate --callers --depth 3
```
...
Surface only the consequential structural decisions a change embeds (the apex of the review brief): a ranked, capped (3 to 5, default 4) set of coupling/boundary, public-API/contract, and dependency decisions, each framed as a judgment question with the routed expert to ask, plus a trade-off clause and the count of in-repo consumers that already depend on the anchor. Runs the same changed-code analysis as `plow review` but emits only the decisions, separable and cheap. Always exits 0 (advisory, never a gate); every decision is suppressible with `// plow-ignore`. Use `--base` / `--changed-since` to pick the comparison point, exactly like `plow audit`.

```bash
plow decision-surface --base main
plow decision-surface --base main --format json --quiet
```
...
```bash
plow explain unused-export
plow explain plow/code-duplication --format json --quiet
```
...
|----------|-------------|
| `<issue-type>` | Issue type token or rule id, for example `unused-export`, `unused-exports`, `plow/unused-dependency`, `high-complexity`, or `code-duplication`. |
...
MCP equivalent: `plow_explain` with required `issue_type`.

---

## `schema`: Capability Manifest

Dumps plow's complete capability manifest as machine-readable JSON (always JSON, regardless of `--format`). The single source of truth for agent introspection.
...
- `environment_variables`: every user-facing `PLOW_*` variable (internal plumbing excluded).
...
Prints the JSON Schema for plow configuration files.

```bash
plow config-schema > schema.json
```
...
```bash
plow rule-pack-schema > rule-pack-schema.json
```

Pack files can also reference the published schema directly: `"$schema": "https://raw.githubusercontent.com/fglogan/genesis-plow/main/rule-pack-schema.json"`.

---

## `license`: Manage Continuous Runtime License

Manage the local JWT used to unlock continuous/cloud runtime monitoring. Single-capture local runtime analysis does not require a license. Verification is fully offline against an Ed25519 public key compiled into the binary. Only `--trial` and `refresh` hit the network (`api.plow.cloud`, 5s connect / 10s total timeout).

```bash
plow license activate --trial --email you@company.com
plow license activate eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9...
plow license activate --from-file ./license.jwt
cat ./license.jwt | plow license activate --stdin
plow license status
...
|------|------|-------------|
| `--trial` | bool | Start a 30-day email-gated trial. Requires `--email`. **Rate-limited to 5 requests per hour per IP** - in CI or behind a shared NAT, start the trial locally and set `PLOW_LICENSE` on the runner. |
...
1. `PLOW_LICENSE` (env var holding the full JWT string)
2. `PLOW_LICENSE_PATH` (env var pointing at a file)
3. `~/.plow/license.jwt` (default; written `chmod 0600` on Unix)
...
| `> 30` | HardFail | Continuous/cloud runtime monitoring is blocked; run `plow license refresh` or start a new trial |

### Actionable error messages

On HTTP error from `api.plow.cloud`, plow parses the `{error, message, code}` envelope and maps known codes to targeted hints:

| Operation + code | CLI message |
|------------------|-------------|
| `refresh` + `token_stale` | `your stored license is too stale to refresh. Reactivate with: plow license activate --trial --email <addr>` |
| `refresh` + `invalid_token` | `your stored license token is missing required claims. Reactivate with: plow license activate --trial --email <addr>` |
| `refresh` or `trial` + `unauthorized` | `authentication failed. Reactivate with: plow license activate --trial --email <addr>` |
| `trial` + `rate_limit_exceeded` | `trial creation is rate-limited to 5 per hour per IP. Wait an hour or retry from a different network (in CI, start the trial locally and set PLOW_LICENSE on the runner).` |
...
|---------|---------|--------|
| `PLOW_LICENSE_SKEW_TOLERANCE_SECONDS` | `86400` (24h) | Overrides the tolerance window applied to the `iat` claim. Lenient parsing: unset / empty / unparsable / negative all fall back to the default. |
...
| `7`  | Network failure or non-success HTTP status from `api.plow.cloud` |

---

## `telemetry`: Opt-in Product Telemetry

Manage opt-in, off-by-default product telemetry that helps prioritize agent, CI, MCP, and editor workflows. Plow never collects repository names, file paths, package or dependency names, source code, config values, environment variable names or values, raw command lines, or raw errors. Hashing those values is not used as a workaround.

```bash
plow telemetry status              # effective state, source, and config path
plow telemetry enable              # opt in (user action only; agents must not run this)
plow telemetry disable             # opt out
plow telemetry inspect --example   # print an example payload + field purposes
```
...
- **Off by default.** Precedence: `DO_NOT_TRACK` / `PLOW_TELEMETRY_DISABLED` (kill switches) > `PLOW_TELEMETRY_DEBUG` (forces inspect) > `PLOW_TELEMETRY` env > CI (off unless `PLOW_TELEMETRY` is set) > user config (`plow telemetry enable/disable`) > off.
- **CI is off** unless `PLOW_TELEMETRY` is explicitly set in that CI environment; a local `enable` never turns on org CI telemetry.
- **Decision status:** `plow telemetry status --format json` includes `explicit_decision`. `false` means the user may have only seen the notice; `true` means `telemetry enable` or `telemetry disable` was explicitly run.
- **Transport:** when enabled, one small JSON event is POSTed to `https://api.plow.cloud/v1/telemetry/events` (override with `PLOW_API_URL`), no auth token, no cookies, on a background thread so it does not delay your command. Delivery is best-effort; errors never change output or exit code.
- **Agent source:** wrappers may set `PLOW_AGENT_SOURCE=<allowlisted-value>` so an enabled run is attributed correctly. Allowlist: `codex`, `claude_code`, `cursor`, `copilot`, `opencode`, `aider`, `roo`, `windsurf`, `gemini` (aliases `gemini_cli`/`antigravity`), `cline`, `continue`, `zed`, `goose`, `other_known`, `unknown`, `none`. Setting it never enables telemetry and uploads no codebase content.
...
- `coverage setup` - resumable state machine that wires sidecar installation, framework-aware coverage recipe writing, optional license activation for continuous monitoring, and automatic handoff into `plow health --runtime-coverage`.
- `coverage analyze` - focused runtime coverage analysis. Local mode reads `--runtime-coverage <path>`; cloud mode requires explicit `--cloud`, `--runtime-coverage-cloud`, or `PLOW_RUNTIME_COVERAGE_SOURCE=cloud` and never triggers from `PLOW_API_KEY` alone.
- `coverage upload-inventory` - push a static function inventory to plow cloud so the dashboard can surface `untracked` functions (those in the codebase but never called at runtime).

```bash
plow coverage setup                         # interactive
plow coverage setup --yes                   # accept all prompts
plow coverage setup --non-interactive       # print instructions, do not prompt
plow coverage setup --yes --json            # agent-readable JSON, no prompts/writes/installs/network
plow coverage setup --yes --json --explain  # add _meta field docs, enums, warnings, docs URL

plow coverage analyze --runtime-coverage ./coverage --format json
plow coverage analyze --cloud --repo owner/repo --format json

plow coverage upload-inventory              # infers project-id, git-sha, API key
plow coverage upload-inventory --dry-run    # print what would be uploaded, exit 0

plow coverage upload-source-maps --dir dist           # upload build source maps from CI
plow coverage upload-source-maps --dry-run            # print maps and fileName values, no network
```
...
2. **Sidecar discovery** - resolves `PLOW_COV_BIN`, `PLOW_COV_BINARY_PATH`, platform-package binaries in npm/bun/pnpm layouts, project-local `node_modules/.bin/plow-cov`, package-manager bin, `~/.plow/bin/plow-cov`, and `PATH`. When an explicit env path is set but points to a non-existent file, setup errors fast instead of falling through.
...
4. **Handoff** - if `./coverage/coverage-final.json` or a V8 coverage directory already exists, setup runs `plow health --runtime-coverage <path>` directly.
...
| `--api-key <KEY>` | string | `$PLOW_API_KEY` | Plow cloud bearer token, used only after explicit cloud opt-in. |
| `--api-endpoint <URL>` | string | `$PLOW_API_URL` or `https://api.plow.cloud` | Override for staging / on-prem. |
| `--repo <OWNER/REPO>` | string | `$PLOW_REPO`, then parsed git origin | Repository whose latest cloud runtime facts should be pulled. Slashes are percent-encoded as one route segment. |
...
| `--production` | bool | false | Run analyze in production mode, matching `plow health --production`. Filters out test files and dev-only code paths before merging runtime data. |
| `--min-invocations-hot <N>` | integer | 100 | Hot-path classification threshold. Functions invoked at least N times during the captured window are classified as hot. Mirrors the same flag on `plow health --runtime-coverage`. |
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--api-key <KEY>` | string | `$PLOW_API_KEY` | Plow cloud bearer token. Generate at `https://plow.cloud/settings#api-keys`. **Prefer `$PLOW_API_KEY` on shared CI runners**: `--api-key` on the command line may be visible to other processes via `ps`. |
| `--api-endpoint <URL>` | string | `$PLOW_API_URL` or `https://api.plow.cloud` | Override for staging / on-prem. |
...
| `--exclude-paths <GLOB>` | glob | none | Additional globs to skip (repeatable), applied after the configured plow ignore rules. |
...
- `PLOW_COV_BIN` - explicit override for the sidecar binary (for `setup`). Wins over all other discovery paths. Must point to an existing file.
- `PLOW_API_KEY` - plow cloud bearer token (for `upload-inventory` and `upload-source-maps`). Overridden by `--api-key` for `upload-inventory`; `upload-source-maps` reads only the env var so secrets stay out of argv.
- `PLOW_API_URL` - base URL for cloud calls. Overridden by `--api-endpoint`.
- `PLOW_CA_BUNDLE` - PEM certificate bundle for cloud calls. Relative paths resolve from the process cwd. The bundle replaces default WebPKI roots, so private-CA runners should pass a complete bundle that includes public roots plus the private CA.
...
| `--endpoint <URL>` | string | `$PLOW_API_URL` or `https://api.plow.cloud` | Override for staging / on-prem. |
...
Prints the loaded config file path and the resolved config (with `extends` merged) as JSON. Useful for verifying which config plow picked up, especially in monorepos.

```bash
plow config            # path on first line, JSON below
plow config --path     # only the path (scriptable)
```
...
| `--no-production` | `bool` | `false` | Force production mode OFF for every analysis, overriding a project config's `production: true` (and `PLOW_PRODUCTION`). Conflicts with `--production` |
...
| `--production-dead-code` / `--production-health` / `--production-dupes` | `bool` | `false` | Per-analysis production mode for bare combined runs and `plow audit`. Per-analysis env vars `PLOW_PRODUCTION_DEAD_CODE`/`HEALTH`/`DUPES` mirror these flags. Per-analysis env beats global `PLOW_PRODUCTION`. |
...
| `--regression-baseline` | `string` | - | Path to regression baseline file (default: `.plow/regression-baseline.json`) |
...
| `--trend` | `bool` | `false` | Compare current health metrics against saved snapshot. Implies `--score`. Shows per-metric deltas with directional indicators. Requires at least one saved snapshot in `.plow/snapshots/` |
| `--save-snapshot` | `string` | - | Save vital signs snapshot for trend tracking. Default path: `.plow/snapshots/<timestamp>.json`. Forces file-scores + hotspot computation |
| `--coverage` | `string` | - | Path to Istanbul coverage data for exact CRAP scores in combined mode. Also settable via `PLOW_COVERAGE` or `health.coverage` |
| `--coverage-root` | `string` | - | Absolute prefix to strip from Istanbul file paths in combined mode. Also settable via `PLOW_COVERAGE_ROOT` or `health.coverageRoot` |
...
| `--save-snapshot` | `string` | - | Save a vital signs snapshot for trend tracking in combined mode. Provide a path or omit for the default `.plow/snapshots/` location |
| `--coverage` | `string` | - | Path to Istanbul coverage data for exact CRAP scores in combined mode. Also settable via `PLOW_COVERAGE` or `health.coverage` |
| `--coverage-root` | `string` | - | Absolute prefix to strip from Istanbul file paths in combined mode. Also settable via `PLOW_COVERAGE_ROOT` or `health.coverageRoot` |
...
|----------|-------------|
| `PLOW_FORMAT` | Default output format. CLI `--format` overrides. |
| `PLOW_QUIET` | Set to `1` to suppress progress. CLI `--quiet` overrides. |
| `PLOW_BIN` | Path to plow binary (used by the MCP server). |
| `PLOW_TIMEOUT_SECS` | MCP server subprocess timeout in seconds (default: `120`). Increase for very large codebases. |
| `PLOW_EXTENDS_TIMEOUT_SECS` | Timeout for fetching remote config inheritance in seconds (default: `5`). Do not raise this for untrusted sources. |
| `PLOW_CACHE_DIR` | Override the persistent extraction cache directory. Wins over `cache.dir`. Useful for read-only checkouts or CI cache volumes. `--no-cache` disables this knob. |
| `PLOW_CACHE_MAX_SIZE` | Maximum on-disk extraction cache (`.plow/cache.bin`) size in megabytes (default: `256`). Triggers LRU eviction when crossed. Wins over `cache.maxSizeMb` config field. Intended for CI runners with disk quotas. `--no-cache` short-circuits this knob. |
| `PLOW_COVERAGE` | Path to Istanbul coverage data for exact CRAP scoring in `health`, `audit`, and bare `plow`. |
| `PLOW_COVERAGE_ROOT` | Absolute coverage-data prefix to strip before matching Istanbul paths in `health`, `audit`, and bare `plow`. |
| `PLOW_AUDIT_BASE` | Pin the `plow audit` comparison base when `--base` / `--changed-since` is unset (precedence: flag > env > auto-detect). Escape hatch for the agent gate and forks, e.g. `PLOW_AUDIT_BASE=upstream/main`. When unset, audit auto-detects the `git merge-base` against the branch's upstream or the remote default. A malformed value exits 2. |
| `PLOW_AUDIT_CACHE_MAX_AGE_DAYS` | Max age (in days since last reuse or fresh create) of a persistent reusable `plow audit` base-snapshot worktree cache. Older entries are reclaimed at the top of the next `plow audit` invocation (default: `30`). Wins over `audit.cacheMaxAgeDays` config field. `0` disables the GC; invalid values silently fall back to config / default. |
| `PLOW_UPDATE_CHECK` | Set to `off`, `0`, `false`, `disabled`, or `no` to disable the human-TTY upgrade nudge and its background latest-version check. `DO_NOT_TRACK`, `PLOW_TELEMETRY_DISABLED`, and CI also suppress it. |
| `PLOW_SUGGESTIONS` | Set to `off`, `0`, `false`, `no`, or `disabled` to suppress the top-level `next_steps[]` array of read-only follow-up commands in JSON output (and the human `Next:` line on bare `plow`). Default on. Inherited by the MCP-spawned CLI, so it disables `next_steps` on MCP responses too. Useful for CI consumers that snapshot-diff raw `--format json`. |
| `PLOW_COMMAND` | GitLab CI: command to run (default: `dead-code`). |
| `PLOW_FAIL_ON_ISSUES` | GitLab CI: set to `true` to exit 1 if issues found. |
| `PLOW_CHANGED_SINCE` | GitLab CI: git ref for incremental analysis. Auto-detected in MR pipelines. |
| `PLOW_COMMENT` | GitLab CI: set to `true` to post MR summary comments. |
| `PLOW_REVIEW` | GitLab CI: set to `true` to post inline code review comments on MR diffs. |
| `PLOW_REVIEW_GUIDANCE` | Add collapsed "What to do" guidance blocks to `review-github` / `review-gitlab` inline comments. |
| `PLOW_SUMMARY_SCOPE` | Sticky PR/MR summary scope for `pr-comment-github` / `pr-comment-gitlab`: `all` (default) keeps project-level findings outside the diff; `diff` applies the diff filter to those findings too. Inline review comments are unaffected. |
| `PLOW_SCORE` | GitLab CI: set to `true` to compute health score in combined mode. Enables health delta header in MR comments. |
| `PLOW_TREND` | GitLab CI: set to `true` to compare current health metrics against saved snapshot. Implies `PLOW_SCORE`. |
| `PLOW_EXTRA_ARGS` | GitLab CI: additional CLI flags passed through to plow. |
| `PLOW_VERSION` | GitLab CI: plow version to install. Empty (default) reads the project's `package.json` `plow` dependency, then falls back to `latest`; set explicitly to override the local pin. |
| `PLOW_SKIP_BINARY_VERIFY` | Skip Ed25519 + SHA-256 verification of platform binaries on first invocation of `plow`, `plow-lsp`, or `plow-mcp` (and during the GitHub Action installer). Set to `1`, `true`, or `yes` ONLY when deliberately replacing the published binary (source builds, airgapped mirrors, signed-repack registries). The skip is recorded in `plow --version` output as `verified: skipped (PLOW_SKIP_BINARY_VERIFY is set)` so it stays visible in CI logs and vendor audits. Never set in regular CI; use the published binary or the documented out-of-band verification recipe in [`SECURITY.md`](https://github.com/fglogan/genesis-plow/blob/main/SECURITY.md) instead. |
| `PLOW_VERIFY_CACHE_DIR` | Override where the lazy-verify sentinel file is written. Cascade is platform-pkg-dir, then this override, then `$XDG_CACHE_HOME/plow/sentinels/` (Linux/macOS) or `%LOCALAPPDATA%\plow\sentinels\` (Windows). Useful when the platform pkg dir is read-only (yarn PnP, Docker layered images, pnpm verify-store). |
| `PLOW_VERIFY_LOG` | Set to `1`, `true`, or `yes` to emit one structured stderr line per verify outcome (`plow-verify outcome=ok cache=hit sentinel=...`). Off by default so MCP stdout/stderr stay clean; enable for CI diagnostic logs. |
| `PLOW_TELEMETRY` | Opt-in product telemetry mode, off by default: `off`/`on`/`inspect` (plus `0`/`1`/`true`/`false`/`disabled`/`enabled`/`debug`/`log`). `inspect` prints the exact payload to stderr without sending. Wins over the user telemetry config. |
| `PLOW_TELEMETRY_DISABLED` | Admin/fleet telemetry kill switch (top precedence, with `DO_NOT_TRACK`). Truthy (`1`/`true`/`yes`/`on`) hard-disables telemetry and refuses `plow telemetry enable`. |
| `PLOW_TELEMETRY_DEBUG` | Forces inspect mode; outranks `PLOW_TELEMETRY`. |
| `DO_NOT_TRACK` | Honored as a top-precedence telemetry kill switch (consoledonottrack.com convention). |
| `PLOW_AGENT_SOURCE` | Declare the calling agent for telemetry classification (only used when telemetry is enabled; never enables it): `codex`, `claude_code`, `cursor`, `copilot`, `opencode`, `aider`, `roo`, `windsurf`, `gemini` (aliases `gemini_cli`/`antigravity`), `cline`, `continue`, `zed`, `goose`, `other_known`, `unknown`, `none`. Unrecognized values are ignored. |
...
Set `PLOW_FORMAT=json` and `PLOW_QUIET=1` in your agent environment to avoid passing flags on every invocation.
...
The HTTP layer mirrors the bash `gh_api_retry` / `curl_retry` helpers: `PLOW_API_RETRIES` (default 3) caps attempts; `PLOW_API_RETRY_DELAY` (default 2) sets the floor delay; server-supplied `Retry-After` overrides the floor on 429 responses.

---

## CI Integration

- **GitHub Actions**: `uses: fglogan/genesis-plow@v2` - supports SARIF upload to Code Scanning, inline PR annotations (`annotations: true`), PR comments, all commands. Annotations use workflow commands (no Advanced Security required); limit with `max-annotations` (default 50). Set `score: true` to compute health score and enable the health delta header in PR comments
- **GitLab CI**: include `ci/gitlab-ci.yml` template and extend `.plow` - generates Code Quality reports via `--format codeclimate` / `--format gitlab-codequality` (inline MR annotations), rich MR comments, code review comments, all commands. Use `plow ci-template gitlab --vendor` when runners cannot reach `raw.githubusercontent.com`; commit the generated `ci/` and `action/` files and use GitLab's local include syntax. Variables use `PLOW_` prefix (e.g., `PLOW_COMMAND`, `PLOW_FAIL_ON_ISSUES`). Set `PLOW_SCORE: "true"` to compute health score; `PLOW_TREND: "true"` to compare against saved snapshots
- **Any CI**: `npx plow --ci` - equivalent to `--format sarif --fail-on-issues --quiet`

### GitLab CI Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `PLOW_COMMAND` | `dead-code` | Command to run (`dead-code`, `dupes`, `health`, or default combined) |
| `PLOW_FAIL_ON_ISSUES` | `false` | Exit 1 if issues found |
| `PLOW_CHANGED_SINCE` | auto | Git ref for incremental analysis. Auto-detected in MR pipelines (`origin/$CI_MERGE_REQUEST_TARGET_BRANCH_NAME`) |
| `PLOW_COMMENT` | `false` | Post a summary comment on the MR with findings |
| `PLOW_REVIEW` | `false` | Post inline code review comments on MR diff lines where issues were found |
| `PLOW_REVIEW_GUIDANCE` | `false` | Add collapsed "What to do" guidance blocks to inline review comments |
| `PLOW_SUMMARY_SCOPE` | `all` | Sticky summary scope: `all` keeps project-level findings outside the diff; `diff` applies the diff filter to those findings too |
| `PLOW_SCORE` | `false` | Compute health score (0-100 with letter grade) in combined mode. Enables the health delta header in MR comments |
| `PLOW_TREND` | `false` | Compare current health metrics against saved snapshot. Implies `PLOW_SCORE`. Shows per-metric deltas |
| `PLOW_EXTRA_ARGS` | - | Additional CLI flags passed through to plow |
| `GITLAB_TOKEN` | - | Project access token with `api` scope (required for `PLOW_COMMENT` and `PLOW_REVIEW`). Alternatively, enable job token API access |
...
**Auto `--changed-since` in MR pipelines**: When running in a merge request pipeline, the template automatically sets `--changed-since origin/$CI_MERGE_REQUEST_TARGET_BRANCH_NAME` unless `PLOW_CHANGED_SINCE` is explicitly set. This scopes analysis to files changed in the MR without manual configuration.
...
```json
{
  ...
  "unused_exports": [{ "path": "src/utils.ts", "name": "unusedFn", "line": 42, "actions": [{"type": "remove-export", "auto_fixable": true, "description": "Remove the unused export from the public API"}, {"type": "suppress-line", "auto_fixable": false, "description": "Suppress with an inline comment above the line", "comment": "// plow-ignore-next-line unused-export"}] }],
  ...
  "re_export_cycles": [{ "files": ["src/api/index.ts", "src/api/internal/index.ts"], "kind": "multi-node", "actions": [{ "type": "fix", "kind": "refactor-re-export-cycle", "auto_fixable": false, "description": "Remove one `export * from` (or `export { ... } from`) statement on any one member to break the cycle" }, { "type": "suppress-file", "kind": "suppress-file", "auto_fixable": false, "comment": "// plow-ignore-file re-export-cycle" }] }],
...
| `auto_fixable` | bool | yes | `true` if `plow fix` handles this action automatically. Evaluated PER FINDING, not per action type: the same `type` may carry `true` on one finding and `false` on another when per-instance guards in the applier discriminate. Filter on this bool of each individual action, not on `type` alone. |
...
```json
{
  ...
  "actions": [
    ...
    },
    {
      "type": "add-to-config",
      "auto_fixable": false,
      "description": "Add to ignoreDependencies in plow config",
      "config_key": "ignoreDependencies",
      "value": "autoprefixer",
      "value_schema": "https://raw.githubusercontent.com/fglogan/genesis-plow/main/schema.json#/properties/ignoreDependencies/items"
    }
  ]
}
```
...
- `add-to-config` for `ignoreExports` (duplicate-exports): `true` when `plow fix` can safely apply the action, which today means EITHER a plow config file already exists OR no config exists and the working directory is NOT inside a monorepo subpackage. In the second case the applier creates `.plowrc.json` using `plow init`'s framework-aware scaffolding and layers the new rules on top. `false` inside a monorepo subpackage with no workspace-root config (the applier refuses to fragment per-package configs). Pass `--no-create-config` to `plow fix` from pre-commit hooks, CI bots, and `plow watch` to opt out of the create-fallback; the action then surfaces with `auto_fixable: false`.
...
Health findings (`plow health` JSON output) include an `actions` array. Primary action selection is formula-aware: the rule first checks whether full coverage CAN bring CRAP under threshold (CRAP bottoms out at `cyclomatic` at 100% coverage, so `cyclomatic < maxCrap` means coverage is a viable remediation), then uses `coverage_tier` to choose the description.
...
### Combined output (`plow` with no subcommand)

When running `plow` with no subcommand (all analyses), the JSON output combines results from all enabled analyses:
...
Config files are searched in priority order: `.plowrc.json` > `.plowrc.jsonc` > `plow.toml` > `.plow.toml`. Both `.plowrc.json` and `.plowrc.jsonc` are parsed as JSON-with-comments; the `.jsonc` extension lets editors auto-detect JSONC syntax highlighting.

### JSON Format (`.plowrc.json` / `.plowrc.jsonc`)
...
```jsonc
{
  ...
  // Extraction cache settings. PLOW_CACHE_DIR overrides cache.dir.
  "cache": {
    "dir": "/tmp/plow-cache",
    "maxSizeMb": 256
  },
  ...
  "extends": ["./base-config.json", "npm:@my-org/plow-config"],
...
| Comment | Effect |
|---------|--------|
| `// plow-ignore-next-line` | Suppress any issue on the next line |
| `// plow-ignore-next-line unused-export` | Suppress specific issue type |
| `// plow-ignore-file` | Suppress all issues in a file |
| `// plow-ignore-file unused-export` | Suppress specific issue type file-wide |
