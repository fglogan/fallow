...
test("describeSigMissing gives upgrade guidance for a pre-signing version", () => {
  const base = { ok: false, code: "sig-missing", message: "signature not found at /x/plow.sig" };
...
test("verifyInstalled threads the resolved version into the sig-missing message", async (t) => {
  const { privateKey, rawPub } = makeKeypair();
  const preDir = makePlatformDir(privateKey, { skipSigFor: "plow-mcp" });
  const eraDir = makePlatformDir(privateKey, { skipSigFor: "plow-mcp" });
  t.after(() => {
    cleanup(preDir);
    cleanup(eraDir);
  });
