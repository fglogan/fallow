...
Review changes to plow's LSP server. This is the language server that editors connect to via stdio. It must be editor-agnostic (VS Code, Neovim, Helix, Zed all consume it).
...
2. **Diagnostic design**: Each diagnostic code maps to a plow issue type. Codes must be stable (editors save filter preferences by code). Severity mapping: unused-file/export/dep -> Warning (not Error, since they're not build failures)
...
7. **Custom notifications**: `plow/analysisComplete` must include stable fields. Clients may depend on the summary structure
