...
Review changes to plow's MCP (Model Context Protocol) server. This is how AI agents (Claude Code, Cursor, Copilot) interact with plow programmatically.
...
4. **Error handling**: Errors must return structured JSON (not plain text). Include actionable guidance ("config file not found at X, run `plow init` to create one")
5. **Timeout handling**: Long-running analyses must respect `PLOW_TIMEOUT_SECS`. Document expected durations for different project sizes
...
8. **Binary resolution**: `PLOW_BIN` env var, `node_modules/.bin/plow` fallback, PATH lookup. Error messages must guide the user to install plow
