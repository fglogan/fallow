...
Review changes to plow's GitLab CI integration. This is an includable CI template that teams add to their `.gitlab-ci.yml`.

## What to check

1. **Template correctness**: `.plow` job definition must be extensible via `extends:`. Variables must use `PLOW_` prefix consistently. Stage assignment should be configurable
2. **Variable documentation**: Every `PLOW_*` variable needs a clear description. Defaults must be sensible for the common case
...
7. **MR review discussions**: Inline discussions must target valid diff positions. Suggestion blocks must use GitLab's specific syntax. Respect `PLOW_MAX_COMMENTS` limit
8. **Comment deduplication**: Previous plow comments should be found and updated, not duplicated. Use a marker/watermark pattern
...
- Plow CLI behavior (review the CI layer, not the tool)
