...
Starting with v1.0, plow follows [semantic versioning](https://semver.org/).
...
- **Config file names**: `.plowrc.json`, `.plowrc.jsonc`, `plow.toml`, `.plow.toml`
...
- **Inline suppression comment syntax**: `plow-ignore-next-line`, `plow-ignore-file`
...
- **Document-root structure**: every object-shaped `--format json` envelope covered by the typed root schema (`PlowOutput`) carries a top-level `kind` discriminator. Consumers should branch on `kind` instead of probing for unique field presence. The authoritative set of typed root kinds lives in `docs/output-schema.json` and includes command envelopes such as `dead-code`, `health`, `dupes`, `combined`, `audit`, `explain`, `feature-flags`, `inspect_target`, `trace`, `coverage-setup`, `coverage-analyze`, `list-boundaries`, `review-envelope`, and `review-reconcile`. Tagged root envelopes are now the only supported object-shaped JSON contract. The CLI `check` command is a legacy alias for `dead-code`; new JSON discriminators use the canonical `dead-code` name. `CodeClimateOutput` stays as a sibling root branch because the Code Climate / GitLab Code Quality spec requires a bare JSON array at the root; discriminate it by checking whether the document root is an array. Helper/spec JSON roots outside `PlowOutput`, such as `fix`, `plow config`, non-boundary `plow list` modes, SARIF, CodeClimate, telemetry, and baseline/config files written by plow, are not part of this envelope contract.
...
```
https://raw.githubusercontent.com/fglogan/genesis-plow/main/docs/output-schema.json
```
...
if (!validate(plowOutput)) {
  console.error(validate.errors);
  process.exit(1);
}
```

For TypeScript types generated from the schema, see `npm/plow/types/output-contract.d.ts` (mirrored to `editors/vscode/src/generated/output-contract.d.ts`). Both are regenerated from `docs/output-schema.json` via `cd editors/vscode && pnpm run codegen:types`.

#### TypeScript bare-name backwards-compat aliases

The schema-derive ladder ([#384](https://github.com/fglogan/genesis-plow/issues/384), [#408](https://github.com/fglogan/genesis-plow/issues/408), [#409](https://github.com/fglogan/genesis-plow/issues/409)) wrapped every bare finding type in a `*Finding` envelope (`UnusedExport` to `UnusedExportFinding`, `CloneGroup` to `CloneGroupFinding`, etc.). The wrappers flatten the bare finding's fields via Rust's `#[serde(flatten)]` and add `actions[]` (and, where the wrapper participates in `plow audit` attribution, the optional `introduced` flag), so the JSON wire shape is byte-identical.

`json-schema-to-typescript` drops the orphan inner definitions when every field is subsumed by a flattening parent (even with `unreachableDefinitions: true`), so the bare names disappear from the generated `.d.ts` unless they are aliased back explicitly. The npm-published `plow/types` subpath (`npm/plow/types/output-contract.d.ts`) carries an alias for every wrapper so external consumers importing the bare names continue to compile. The full list lives at the end of the generated file under the `// Backwards-compat aliases` section, with per-alias JSDoc explaining the migration history.

**Stability commitment**: the bare-name aliases are part of plow's v2.x stable surface. They are scheduled for removal alongside the kind-tagged `PlowOutput` major bump ([#413](https://github.com/fglogan/genesis-plow/issues/413)). The removal will be preceded by one minor release that adds `@deprecated` JSDoc to each alias and a CHANGELOG headline announcing the timeline. New code that consumes plow's JSON output should import the `*Finding` wrapper names directly so the major bump is a no-op for the consumer.

### Rust programmatic API

- **Primary API crate**: `plow-api` owns programmatic option, error, typed output, and JSON serialization contracts. Rust embedders should call the typed `run_*` entry points (`run_dead_code`, `run_duplication`, `run_circular_dependencies`, `run_boundary_violations`, `run_feature_flags`, `run_health`) and serialize only at their own protocol boundary via the matching `serialize_*_programmatic_json` function when needed.
- **JSON protocol serializers**: `serialize_*_programmatic_json` functions remain exported from `plow-api` for CLI, MCP, NAPI, and custom protocol adapters. They are serializers over typed `run_*` contracts, not alternate Rust runtime entry points.
- **No Rust compatibility adapter crate**: `plow-programmatic-cli` has been removed. New and existing Rust embedders should depend on `plow-api` directly.
...
- `plow-api::runtime_json`: keep JSON protocol serializers only. New command families must expose typed `run_*` output first and add JSON only at protocol boundaries.

### CLI interface

- **Subcommands**: `dead-code` (legacy alias: `check`), `dupes`, `health`, `audit`, `security`, `explain`, `fix`, `watch`, `init`, `hooks`, `setup-hooks`, `migrate`, `list`, `schema`, `config-schema`, `plugin-schema`, `config`, `coverage`, `license`, `ci`. `security` is opt-in (the `security-client-server-leak` rule defaults to `off`); its findings never appear under bare `plow` or `audit`.
- **`coverage` subcommands**: `setup`, `analyze`, `upload-source-maps`, `upload-inventory`. `analyze` accepts `--runtime-coverage <path>` for local mode and `--cloud` / `--runtime-coverage-cloud` (or `PLOW_RUNTIME_COVERAGE_SOURCE=cloud`) for explicit cloud-pull; `PLOW_API_KEY` alone never selects cloud mode.
...
- **Default behavior**: bare `plow` (no subcommand) runs dead-code + dupes + health combined
- **Exit codes**: 0 (success/no errors), 1 (issues with error severity found), 2 (runtime error). `plow audit` defaults to `--gate new-only`, so inherited error-severity findings in changed files can be reported with exit 0; use `--gate all` to fail on every finding in changed files. `plow security --gate new` and `plow security --gate newly-reachable` add exit code **8**, dedicated to a security candidate matching the selected gate mode (changed-line candidate or newly entry-reachable candidate). A gate that cannot compute its required diff or base tree exits 2, not 8. The code is stable so pipelines can pin it (for example GitLab `allow_failure: exit_codes: [8]`). The official GitHub Action exposes the same gate through `security-gate`, and the GitLab template exposes it through `PLOW_SECURITY_GATE`.
- **Global flags**: `--format`, `--config`, `--workspace`, `--production`, `--no-production` (force production mode off, overriding a project config's `production: true`; conflicts with `--production`), `--baseline`, `--save-baseline`, `--no-cache`, `--threads`, `--changed-since` (alias: `--base`), `--churn-file` (import a `plow-churn/v1` JSON change-history file for hotspots/ownership/targets on non-git VCS), `--performance`, `--explain`, `--ci`, `--fail-on-issues`, `--sarif-file`, `--output-file` (alias: `-o`; write the report to a file instead of stdout, for any `--format`), `--fail-on-regression`, `--tolerance`, `--regression-baseline`, `--save-regression-baseline`, `--summary`, `--group-by` (owner, directory, package, section), `--include-entry-exports`, `--max-file-size` (skip source files larger than N megabytes at discovery, default 5, `0` disables; declaration files are always analyzed), `--dupes-mode`, `--dupes-threshold`, `--dupes-min-tokens`, `--dupes-min-lines`, `--dupes-min-occurrences`, `--dupes-skip-local`, `--dupes-cross-language`, `--dupes-ignore-imports`, `--dupes-no-ignore-imports` (count module wiring in combined mode; opt out of the default exclusion)
- **Per-analysis production flags**: `--production-dead-code`, `--production-health`, `--production-dupes` (bare combined mode and `plow audit`)
...
- **Environment variables**: `PLOW_FORMAT`, `PLOW_QUIET`, `PLOW_BIN`, `PLOW_TIMEOUT_SECS`, `PLOW_EXTENDS_TIMEOUT_SECS`, `PLOW_COVERAGE`, `PLOW_COVERAGE_ROOT`, `PLOW_CACHE_DIR`, `PLOW_API_URL`, `PLOW_API_KEY`, `PLOW_CA_BUNDLE`, `PLOW_PRODUCTION`, `PLOW_PRODUCTION_DEAD_CODE`, `PLOW_PRODUCTION_HEALTH`, `PLOW_PRODUCTION_DUPES`, `PLOW_REVIEW_GUIDANCE`, `PLOW_SUMMARY_SCOPE`, `PLOW_AUDIT_CACHE_MAX_AGE_DAYS`, `PLOW_UPDATE_CHECK`, `PLOW_MAX_FILE_SIZE` (per-file size limit in megabytes, mirrors `--max-file-size`; `0` disables), `PLOW_SUGGESTIONS` (set to `off`/`0`/`false`/`no`/`disabled` to suppress the `next_steps[]` array in JSON output and the human `Next:` line; default on)
...
- **Generated hook-script env vars**: `PLOW_GATE_MIN_VERSION` (consumed by `.claude/hooks/plow-gate.sh` written by `plow hooks install --target agent` / `plow setup-hooks`; controls the minimum plow version the gate accepts, default `2.46.0`, empty string disables)
...
- **Internal crate APIs**: `plow-core`, `plow-config`, etc. are not public API
