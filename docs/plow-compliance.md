# Plow Compliance Happy Path

Plow compliance defines the maintainability and codebase-quality expectations for a repository.
...
This is the shortest path from "we installed plow" to "this repo is plow-compliant."

For most teams, **plow-compliant** means:

- `plow health` shows **`Above threshold: 0`** for the thresholds the team chose on purpose
...
- if the team uses staged adoption, `plow audit` passes on the current change set with intentionally chosen per-analysis baselines

## Compliance signals

In priority order:

1. **Quality score** -- `plow health --score` reaches the team's agreed minimum
2. **Changed-code risk** -- `plow audit` returns verdict `pass` for the current change set
3. **Complexity and hotspots** -- `plow health` shows `Above threshold: 0`
...
- `.plowrc.json`, `plow.toml`, or inline suppressions only capture intentional cases
...
- `plow audit` is treated as the PR or change-review gate, not the repo-wide cleanup verdict
...
Choose this when the code is supposed to stay, but plow cannot infer that from syntax alone.
...
1. Run `plow` or the relevant repo-wide subcommands to see the current state.
...
4. Re-run plow after each batch.
5. Once the repo-wide state is intentional, wire `plow audit` in as the change-set gate.
...
Keep committed baselines outside `.plow/`; that directory is usually gitignored cache/local state, not a good home for review gates that should travel with the repo. `plow-baselines/` is the recommended default.
...
Use this when you want an agent to make an existing repo plow-compliant.

```text
Make this repository plow-compliant.

Goal:
- repo-wide dead code and duplication findings are either fixed or intentionally excluded
- `plow health` reports `Above threshold: 0` for the thresholds chosen for this repo
- every exclusion is narrow, documented, and kept only when there is a real reason
- if staged adoption is needed, `plow audit` passes with intentionally chosen per-analysis baselines so only new issues fail

Process:
1. Run plow and inspect the current findings.
...
8. If the repo cannot be fully cleaned up in one pass, save per-analysis baselines and use `plow audit` as a gate on new issues while cleanup continues.
9. Re-run plow until the repo reaches the goal state.
